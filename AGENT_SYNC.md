...
**Cockpit next steps (Task 13 onward):**
1. **Task 13 — next** (B-shaped always-abstain stub + `run_backtest_b.py` CLI, §6.1), under the Falsification Discipline (Codex seeds the falsification matrix in RED; independent technical clean/go; evidence-bound). The stub must return the structured abstain (status/reason/required_gate/upstream_run_id/`decision_supported=False`/exit_code=0) and lock the abstain behavior so a future agent must explicitly flip it.
2. **Task 14** — audit tests + inviolate-path SHA baselines + AGENT_SYNC/ledger closeout, then PR.
3. **Tracked follow-up (out-of-scope-by-contract):** the real nflreadr→`NflTruthRow` truth loader + `_compute_bridge_coverage` confirmed-class universe (its own RED). Until then, real-mode Backtest-A runs fail closed.
