...
**Cockpit-cleared next steps (Task 12 onward):**
1. Claude implements Task 12 (artifact writer + `run_backtest_a.py` CLI, §5.9 — the runner wiring Tasks 9–11: `join_bridge_to_realized` + `evaluate_bridge_gates` + `compute_metrics` + `evaluate_b_gate`) against Codex-authored RED tests.
