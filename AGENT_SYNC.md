...
**Cockpit next steps (S4 build complete → ship):**
1. **S4 BUILD COMPLETE** — Tasks 0–14 committed on `feature/subsystem-4-backtest-harness` (local, ahead of origin; full suite 1537/11/0; validate_governance PASS; inviolate paths byte-unchanged vs main). **Remaining = OUTWARD-FACING, pending David's explicit go:** `git push origin feature/subsystem-4-backtest-harness` then `gh pr create` with an audit-trail-enriched body (mirror the S3 PR #55 pattern; list all S4 build commits + the pressure-test/retrospective/Falsification-Discipline history). NOTE: the original Cockpit Process section was reverted on S4 then `main` was merged back in (`d7abbc3`) — so the S4↔main governance-doc delta is clean (S4 carries main's doctrine).
2. **Tracked follow-up (out-of-scope-by-contract, owner = a future increment):** the real nflreadr→`NflTruthRow` truth loader + `_compute_bridge_coverage` confirmed-class universe (its own RED). Until then, real-mode Backtest-A runs fail closed (`nflreadr_truth_unavailable` → metrics null).
