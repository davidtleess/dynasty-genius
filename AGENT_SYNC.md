...
**Cockpit next steps (Task 14 onward):**
1. **Task 14 — next (the last S4 build task):** audit tests + AGENT_SYNC/ledger closeout, under the Falsification Discipline. Per plan §8.8/§11.1: S3 + Phase 10/11/12 inviolate-path SHA-256 baselines byte-check (recorded `INVIOLATE_BASELINE_SHA256_A41E0C6`), mock-data isolation (no mock/ADP/market field names in `prospect_nfl_bridge.py`; no market fields in any S4 schema/output; mock fields allowed only in the `backtest_mock_draft.py` snapshot/run surfaces per §7), no banned David-facing language, `decision_supported=False` recursively, §4.5 coverage-matrix reconciliation, §6.3 acceptance criteria. Then the S4 PR.
