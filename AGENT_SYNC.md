...
**Cockpit next steps (Task 12 onward):**
1. **Task 12 — QUEUED (next; not started; awaiting David's go / fresh session).** Pressure-test blocker is CLOSED (`cfc0461`). Claude implements Task 12 (artifact writer + `run_backtest_a.py` CLI, §5.9 — the runner wiring Tasks 9–11: `join_bridge_to_realized` + `evaluate_bridge_gates` + `compute_metrics` + `evaluate_b_gate`). **Runs under the new Falsification Discipline:** Codex SEEDS the falsification matrix in the RED (all input-class rows — the 9 S4 defects all lived in untested input classes); evidence-bound claims; independent technical clean/go (Codex reviews Claude GREEN with cited break-attempts); fresh-artifact review after each fix; consensus on a separate verified turn. **Task 12 MUST resolve the documented compute_metrics↔evaluate_b_gate data-flow** — build the per-(bucket,position) MAE/coverage structure `evaluate_b_gate` consumes (compute_metrics emits round-level `{n_realized,n_scored}`, a different shape) and decide whether the `metrics` param is wired or formally dropped (rule 8 / D4 note).
2. Tasks 13–14 proceed per plan with §11 cross-impacts in place.
