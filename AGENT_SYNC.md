...
1. Pressure-test blocker is **CLOSED** (`cfc0461`). Claude implements Task 12 (artifact writer + `run_backtest_a.py` CLI, §5.9 — the runner wiring Tasks 9–11: `join_bridge_to_realized` + `evaluate_bridge_gates` + `compute_metrics` + `evaluate_b_gate`). **Task 12 MUST resolve the documented compute_metrics↔evaluate_b_gate data-flow** — build the per-(bucket,position) MAE/coverage structure `evaluate_b_gate` consumes (compute_metrics emits round-level `{n_realized,n_scored}`, a different shape) and decide whether the `metrics` param is wired or formally dropped.
