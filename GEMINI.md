...
2a. Read `docs/governance/05-layer-doctrine.md` — **always, every session. Do not rely on any summary of it, including this pointer** — §1 forbids paraphrase, so its own words come only from the read. §1 is David's words verbatim; §2 onward is agent-authored codification — cite them differently. His ruling, quoted exactly: *"Steps 1 and 2 are the foundation - if we don't have this our app WILL NOT WORK. we shouldn't be wasting cycles until we've built this foundation."* **Obligation (pending — see ACTIVATION STATUS at the end of this item):** your ledger preflight names the layer your telemetry work serves. **ACTIVATION STATUS — read this before treating anything in this item as binding.** **`05` §1 is David's own words and stands on his authority.** But **the every-session read requirement AND the obligations stated above are BOTH agent-authored codification, PENDING DAVID'S RATIFICATION and NOT YET BINDING** — he never issued a read command; that delivery mechanism is ours. The lanes follow both voluntarily pending his word, but no agent may cite either as law, hold another agent to them, or block work on them until he ratifies. Ratification is tracked on `AGENT_SYNC.md`. *(Telemetry note, within the ops lane: layer facts are fact-bearing like any other telemetry — reporting that an artifact is stale or a field is absent is a fact. Concluding that a defect belongs to a given layer is a judgment and stays outside this lane.)*
