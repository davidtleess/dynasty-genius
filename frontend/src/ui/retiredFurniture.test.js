...
// REVIEW-PANEL FIX — the list above is five WHOLE strings, and the register has
// more variants than that. A browser run over the finished branch found a sixth
// one still rendering on Roster Audit ("Context signal — not decision-grade.",
// QbContextSection) and two more on Model Trust, while this guard passed and the
// ticket certified both surfaces clean. A guard that certifies a claim has to be
// at least as wide as the claim, so the PHRASES that make a string a member of
// the repealed register are pinned too, not just five sentences that happened to
// contain them.
//
// The phrases, not the words: "decision-grade" and "decision support" are the
// governance vocabulary David repealed. What survives on purpose is the FACT
// each stamp stood on, said in plain words — "not a proven market-beater", "not
// a claim that it beats the market", "it does not mean the model is proven" —
// none of which matches these patterns, which is the point.
const RETIRED_PHRASES = [/not decision-grade/i, /decision[- ]support/i];
...
describe("DG-111 retired furniture", () => {
  ...
  it("catches any NEW sentence written in the repealed register, not just the five", () => {
    const offenders = [];
    for (const file of authoredSourceFiles(SRC_ROOT)) {
      const body = stripComments(readFileSync(file, "utf8"));
      for (const phrase of RETIRED_PHRASES) {
        const hit = phrase.exec(body);
        if (hit) {
          offenders.push(`${relative(SRC_ROOT, file)} still says "${hit[0]}"`);
        }
      }
    }
    expect(offenders.sort()).toEqual([]);
