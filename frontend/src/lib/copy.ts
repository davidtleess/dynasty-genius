 ...
 * words above already exist for the same three things, so this is a lookup, not
 * a fourth vocabulary.
 */
const BASIS_LANE: Record<string, string> = {
  A: "ENGINE_A",
  B: "ENGINE_B",
  blend: "BLEND_AB",
};

export function basisWord(basis: string | null | undefined): string | null {
  if (basis === null || basis === undefined) return null;
  const lane = BASIS_LANE[basis];
  return lane === undefined ? null : valueWord(lane);
}
...
const TOKEN_SENTENCES: Record<string, string> = {
  ...
  // sentence now says.
  //
  // DG-150 — this used to be a full sentence opening "Our projections were
  // rebuilt on a newer model run". Under the old formula that was near enough: a
  // microsecond timestamp in the provenance hash made a new vintage every
  // morning, so the flag effectively meant "we re-scored". DG-141 (B) took that
  // timestamp and the daily-renewed Sleeper list hash out of the vintage, and
  // what remains fires for at least FOUR causes the payload does not
  // distinguish: a player's captured details moving (team, age, roster status),
  // the compared population changing, a `governance_version` bump (deliberately
  // still hashed — capture driver, _LINEAGE_KEY), and a genuine model rebuild
  ...
  //
  // It is now a FRAGMENT, not a sentence, on David's ruling that the morning
  // status is "glyphs and symbols, not full sentences" (2026-09-04). It asserts
  // the one thing the payload entails and names no cause. The vintage half is
  // NOT repeated here: the receipt line directly above it already says
  // "Projection basis changed within this window." from `model.vintage_changed`,
  // which is the same fact and was always cause-free.
  vintage_changed_no_score_delta: "no compared player moved",
  ...
  // The scope clause is load-bearing and it is the same one this dictionary
  // already uses for `vintage_changed_no_score_delta` ("the players we could
  // compare"). The detector only verifies a position with at least 8 compared
  // players, and the STATUS is global — so on a morning where one position is
  // thin, an unqualified "every score in a position" would assert the uniform
  // move for a position nothing checked. Measured: with TE verified over 12
  // players and QB carrying 5, the detector returns {TE} alone and the status
  // still fires. Say only what was checked.
  model_uniform_factor_per_position:
    "We are not comparing player by player today: in the positions we could check, every score moved by an identical amount, which is a change of units rather than a change in anyone's outlook.",
...
// "paused for today", not "until tomorrow": the weaker form is unconditionally
// true when it renders. "Until tomorrow" is a prediction — well founded, since
// the factor is 1.0 the next morning by construction, but only under the
// assumption of no second declared change, and an assumption is the thing that
// has bitten this product ten times over.
export const SCALE_CHANGED_MASTHEAD =
  "Today's scores are on a new scale. Player-by-player comparison is paused for today.";
