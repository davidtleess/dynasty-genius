...
LEAGUE_PULSE_PHASE_1_DEBT: list[AllowlistEntry] = [
    ...
    # from those same files. The sole surviving legacy references are in the
    # v1-compat shim below (which normalizes stale v1 cards), removed at T4.
    AllowlistEntry(
    ...
    AllowlistEntry(
        ...
        reason='transitional stale league_opportunity.v1 card normalization (drops the removed composite score); removed at Phase 1 T4',
    ),
    AllowlistEntry(
        path='app/api/routes/league_pulse_v1_compat.py',
        token='WAIVER_CANDIDATE',
        reason='transitional stale league_opportunity.v1 card-type mapping to the v2 neutral name; removed at Phase 1 T4',
    ),
    AllowlistEntry(
        path='app/api/routes/league_pulse_v1_compat.py',
