# Dynasty Genius Docs Index

## Mandatory Start

All agents start with:

1. `docs/governance/02-agent-operating-loop.md`
2. `docs/governance/00-product-constitution.md`
3. `docs/governance/01-north-star-architecture.md`
4. `docs/governance/03-code-hygiene-policy.md`
5. `AGENT_SYNC.md`

## Canonical Governance

- `governance/00-product-constitution.md` — analytical doctrine and locked product rulings.
- `governance/01-north-star-architecture.md` — technical architecture and phase sequence.
- `governance/02-agent-operating-loop.md` — required agent workflow, handoffs, and ledger rules.
- `governance/03-code-hygiene-policy.md` — lint scope, enforcement, and unsafe-change guardrails.
- `agent-ledger/` — daily session logs for all agents.

## Historical / Supporting Docs

- `mission.md` — earlier product mission and competitive objective.
- `roadmap.md` — earlier phased delivery plan.
- `model-architecture.md` — earlier two-engine architecture reference.
- `next-sprint.md` — earlier implementation priorities and done criteria.
- `mission-recalibration-2026-04-29.md` — dated recalibration snapshot.
- `codex-review-2026-04-30.md` — independent code review baseline.
- `product-strategy-2026-04-30.md` — Session B product strategy review.
- `decision-output-contracts.md` — David-facing output schemas.
- `review-*.md` — dated review artifacts from prior agent sessions.
- `session-summary-2026-04-30.md` — end-of-day summary.
- `storage-strategy.md` — git vs artifact storage policy.
