...
**Design.**
- Use the existing community-CSV ingest + `MarketSnapshotStore` (`ktc_community_csv`/`dp_archive` modes) to backfill historical FantasyCalc dynasty values at the harness's `_market_snapshot_date(test_year)` targets (the walk-forward folds: test years 2020–2023 → market-snapshot dates **2021/2022/2023/2024-09-08**). Then let the *already-built* `_compute_market_ndcg` + G3 path run.
- **Adapter vs. loader split (W1.4 scope boundary; Codex 2026-05-30).** W1.4 builds the point-in-time-validating **adapter** `scripts/backfill_market_archive.py::backfill_market_archive(archive: list[dict], …)` — it takes an *already-loaded* archive and applies the PIT gates (±7-day window, `updated_at ≤ archive_publish_date`, required fields, int-coercible value, backfill-only source `ktc_community_csv`/`dp_archive`, never `fc_native`), writing via `append_snapshots`. The source-specific **loader/parser** (file/source → `list[dict]`) is a **separate later increment** — its format depends on which archive David supplies, and **the actual archive data is not in the repo** (FC's API serves only *today*; forward W2a collection cannot backfill the past). So the *actual* G3 run + verdict (Step 5) is **gated on David sourcing a point-in-time-correct archive** for those four dates (the §8.4-approved external source). The adapter + harness wiring build and test on synthetic fixtures now. See `[[project_w1_needs_historical_fc_archive]]`.
