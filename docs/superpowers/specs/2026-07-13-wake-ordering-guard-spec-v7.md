...
- **BF-5 (rollback writer):** `scripts/run_league_intelligence_refresh.py:66-80,253-264` backs up and RESTORES both divergence latest files outside G5 — a rollback can place an OLD pair under a NEWER marker. That path is retired; recovery is G3's own mechanism (the next owning generation), never file restoration.
...
- **Scan contract for rows 33/34 (pinned):** scope = `app/`, `src/`, `scripts/` (tests/docs excluded); pattern = the two latest-file basenames; ALLOWLIST = exactly the canonical runner module and the G4 loader module; any other hit fails. (Build-equivalent alternative: a mutation-API boundary directory whose only accessor is loader/runner.)
...
- **#148 health extension (executable schema):** `report_freshness.json`'s market-divergence entry gains `companion_artifacts: [latest_path, coverage_path]` and `verify: {generation_field: "generation_id", output_hash_fields: ["latest_sha256", "coverage_sha256"]}`. The checker (`system_health_models.py`) evaluates **marker status FIRST**: non-ok ⇒ `producer_failure:<reason>` and companion verification is SKIPPED (a degraded marker intentionally carries null generation/hashes; it must never re-report as `marker_hash_malformed`). Only for `status=ok` does it open both companions, verify the three-way `generation_id` match, and recompute both hashes. Failure bases (exact): `generation_mismatch`, `output_hash_mismatch`, `marker_hash_malformed` — REDs for rows 14/15/23/25/26 + the precedence RED (row 44).
...
- **Report schema — the same two strict shapes:** ok-shaped = `generation_id` + both output hashes + `provisional=true`; degraded-shaped = nonempty `reason` + attempt metadata with generation/hashes explicitly null (no-output failures carry no output fields). The provisional flag is PHYSICALLY PERMANENT (marker-last performs no rewrite).
- **Commit is a READER property WITH INTEGRITY:** the ok marker records `report_sha256` over the exact written report bytes; a committed read = same-generation ok marker + report bytes matching `report_sha256` + the PAIR fetched through the G4 exact-byte loader. Self-declared strings alone never establish commitment (mutation would pass). Marker/report `retries_used` agreement is claimed only where both writes succeed (marker authoritative otherwise).
...
- **content_sha256, reproducible (pinned):** SHA-256 over ONE enclosing JSON array of stored-row tuples `[sleeper_player_id, payload_json]` (payload byte-identical as stored), sorted by `sleeper_player_id`, canonical encoding identical to G2 (UTF-8, sort_keys, ensure_ascii=False, separators, allow_nan=False; nonfinite = FATAL). Readers compute from ONE SQLite snapshot (metadata + day rows in a single transaction).
