...
`K = clamp(round(median(promotions across the whole corpus)), 0, 3)` — **median** (robust to a
single outlier in a small sample), **global** (one K for all qualifying QBs), clamped to [0, 3].
`round_threshold_pick` stays at its permissive default (all QBs qualify) for v1.
...
- **David's league rookie drafts (live Sleeper, read-only):** from `league_id 1314363401744416768`,
  ...
  **rookie drafts** (`settings.rounds ≤ 6` excludes the inaugural startup), pull picks →
  `(ff_slot, player, position)` from pick metadata.
...
- `_recommend_k`: `clamp(round(median), 0, 3)` — incl. a negative-median case → 0, a large-median
  case → 3, an empty-promotions case → 0 (+ caveat).
- Rookie-draft filter: `rounds ≤ 6` selected, a 15-round startup excluded.
