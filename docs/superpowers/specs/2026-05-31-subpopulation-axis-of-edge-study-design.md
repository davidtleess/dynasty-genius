...
- **Multiplicity (statistically coherent — Codex #3 + r2 #3):** compute a **two-sided paired bootstrap p-value on `rho_diff`**. The **primary BH family is the per-(axis,slice,position) AGGREGATE tests only** (one p-value per aggregate slice; §9.3 global across those aggregates). **Fold-level p-values are diagnostic only — no `q_value` / `powered_followup_candidate` from them** unless reported as a separately-labeled exploratory fold-level family. Report the BH **`q_value`** descriptively; `q ≤ 0.10` → `powered_followup_candidate=true`, explicitly **hypothesis-generating, not a verdict**. BH corrects p-values, never CIs. **No pass/fail promotion gate is emitted.**
...
- **`src/dynasty_genius/eval/subpopulation_landscape.py`** — pure functions, model-blind, no I/O of model artifacts:
  - `resolve_draft_year(db_playerids_rows) -> {gsis_id: draft_year}` — deterministic de-dup: latest `db_season` per `gsis_id`; raise `ValueError` on conflicting non-null `draft_year` for the same `gsis_id`; require integer `draft_year` (Codex finding #5).
  ...
  - `apply_fdr(aggregate_slice_tests) -> +q` — Benjamini-Hochberg over the **aggregate** per-(axis,slice,position) p-values only (global across aggregates); sets `q_value` + `powered_followup_candidate` (q ≤ 0.10, hypothesis-generating). Fold-level p-values are not part of this family.
  - `aggregate_folds(slice_folds) -> {median_rho_diff, folds_covered, ...}` — aggregate across evaluable folds; preserves the fold-level rows.
...
- **De-duplication (Codex finding #5):** `db_playerids` may carry multiple rows per `gsis_id` across `db_season`. Select the **latest `db_season`** row per `gsis_id`; **raise `ValueError` on conflicting non-null `draft_year`** for the same `gsis_id` (identical/null-vs-value is fine). The selected `db_season` snapshot is recorded in provenance.
...
- **Provenance fields in output:** `draft_year_source="dynastyprocess_db_playerids"`, source path/artifact label, `db_season_snapshot`, `draft_year_coverage_numerator/denominator`, `excluded_missing_draft_year_count`, `invalid_negative_experience_count`.
...
- **FDR coherence + family (Codex #3, r2 #3):** BH applied to bootstrap **p-values** (not CIs), over **aggregate** per-(axis,slice,position) tests only; fold-level p-values carry no `q_value`/`powered_followup_candidate`; `powered_followup_candidate` set at q≤0.10 and labeled hypothesis-generating.
- **Early-career contract:** missing/non-integer `draft_year` → `early_career_axis_unavailable` (no age substitution); `db_playerids` de-dup picks latest `db_season`; **conflicting non-null `draft_year` for one `gsis_id` raises `ValueError`**; <95% coverage → axis/position-fold unavailable. Guards finding #5 + §7.
