...
- **`promote_recommended = true`** (conservative OR; review-prompt ONLY, never auto-promote) if `count_model_supported_players_drifted_gt_5pct > 20` **OR** any modeled route/coverage count changes by ≥ 10 **OR** `p95_abs_value_delta ≥ 1.0`. Thresholds are initial recommendations, tunable post-build.
