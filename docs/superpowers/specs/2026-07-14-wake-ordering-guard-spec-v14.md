# Wake-Ordering Guard — Spec v14 (STANDALONE; post-RED round 13, 2026-07-14)

**Problem (observed 2026-07-12):** the 09:40 divergence runner and the 09:00 FC capture coalesce on wake; the runner read the market store seconds before the fresh capture landed and degraded honestly. **Mechanism (RED-CLEAR, rounds 2–10): bounded polling of the transactional FC SQLite store.** The capture report is rejected as a sentinel. Implementation and schedule are David-gated; this document is the complete normative contract and supersedes v0–v13 entirely. v9's pointer model is REMOVED (it created a dual commit); **the marker is the single commit object.**
...
- **TWO INDEPENDENTLY STABLE MARKER SLOTS (r12-1):** `marker_committed.json` and `marker_attempt.json` are BOTH config-pinned stable paths (neither resolved from the other; the ok schema carries no attempt locator). Committed is replaced only by ok markers (the serving truth); attempt by EVERY terminal. **First-run / absence UX (r13-12, exact):** three distinguishable states — `legacy_unguarded` (migration: unsuffixed serves, flagged); `no_committed_generation` (verified MODEL facts serve normally; market/gap/movement columns dash with "no verified market read yet"; the as_of object carries ONLY the PVO vintage — no invented timestamps); post-activation loss (same dashing + health alarm). Nullable fields exact: market-lane numerics null, `market_lane_status` carries the state, model-lane fields never null for valued players.
- **Serve-stale-DISCLOSED + SCHEDULE-AWARE FAIL-CLOSE (r13-4):** the loader returns `committed_finished_at` + `latest_attempt`. Staleness = ONE schedule-aware function: `due = next_scheduled_run(09:40 America/New_York daily, after=committed_finished_at)`; `overdue = now_utc > due + GRACE(30min)`. Overdue OR a degraded-and-newer attempt ⇒ the market lane DASHES per composition v3.14 **while still displaying the prior vintage date** (dash the comparison, never hide the honest as-of). Known failed attempts dash IMMEDIATELY. All timestamps aware UTC (the schedule converts from ET); `committed_finished_at > now` ⇒ `clock_anomaly`, dash. A missed 09:40 is visible by ~10:10 ET, not 26 hours later.
- **G-ORDER (total, r13-5/6 recut):** history transaction → artifact files (INERT until referenced) → **iff ok: COMMITTED marker → ATTEMPT marker → REPORT**; degraded runs: ATTEMPT marker → REPORT. **Reports write LAST as pure audit artifacts:** they mirror the FINAL terminal (finished_at mirroring is now possible), are named by attempt_id, and are bound by attempt_id + the shared validator — `report_path`/`report_sha256` are REMOVED from markers (nothing writes before its truth is known; no rewrite path exists). Report-write failure after both markers = log+nonzero; markers stand. **Ordering is computable: `commit_seq` (monotonic int) persists IN both markers** (committed = prior seq + 1; attempt carries its candidate). `attempt_lagged` = attempt.seq < committed.seq, in the health enum. First-success-killed-between-markers ⇒ committed B + attempt absent/old = attempt_lagged, benign, self-heals.
- **GC protocol (r13-7 — rebuild-free):** each generation lives in its OWN DIRECTORY `generations/<commit_seq>_<generation_id>/` — whole-generation quarantine is ONE rename to `.trash/`; ordering is the seq prefix; NO index needed: **retention = the directory named by the current committed marker + anything younger than GRACE_GC (24h) by mtime; all else prunable** (past generations matter only until the next commit + grace; inert pre-commit dirs fall to the same age rule). Both markers re-read immediately before acting; partial GC re-quarantines next start. Reader retry: ENOENT ⇒ discard the ENTIRE snapshot, re-read committed marker + both companions, once; second miss fails close.
...
- **PVO coherence (r13-9 — mutually exclusive branches):** (a) bytes EQUAL ⇒ coherent (lineage differs w/ equal bytes ⇒ coherent + informational `pvo_pending_lineage`); (b) bytes DIFFER ∧ current lineage OLDER ⇒ `pvo_regressed` — the lane fail-closes, precedence over all; (c) bytes DIFFER ∧ lineage same-or-newer ⇒ `gap_basis_stale` — the gap nulls, the model serves; same-lineage-different-bytes additionally raises `pvo_content_anomaly` (absorbs the retired pvo_mismatch; row 27 updated). Byte equality is the only sufficiency test (PVO has four upstream inputs); upstream-digest authentication = named optional extension.
- **Trade honesty (r13-10 — equality dissolves):** the gap context renders FROM THE LOADER-SERVED PAIR (captured rows); trade never compares adapter rows to the capture digest because it never uses adapter rows for the gap. The adapter prices assets only, with typed `fetched_at` + `cache_state` ∈ {fresh, cached_within_ttl, stale_cache_served, cold_unavailable, corrupt, future_stamp, naive_stamp} (BF-10); `price_fetch` is null exactly when no fetch occurred. Dual-stamp ("price as fetched X (cache_state) · gap basis as of Y"); "live" is banned.
...
- **Marker `ok` (exact, closed):** `status="ok"` · `schema_version` · `commit_seq` int≥1 · `attempt_id` (== `generation_id`, uuid4) · `generation_dir` (confined) · `latest_sha256`/`coverage_sha256` · the three input digests · `input_pvo_source_as_of` · `input_pvo_snapshot_captured_at` · `input_pvo_kind` · `finished_at` (aware UTC, sampled at terminal write — BF-9) · `retries_used` int≥0 non-bool · `decision_supported=false`. NO `reason`, NO report fields (reports FOLLOW the markers, bound by attempt_id). The degraded shape adds its candidate `commit_seq`, outputs null.
...
- **Reader contract (r13-3, strict):** every live history reader recomputes row_count + the V3 content hash (byte rules restated: one canonical enclosing JSON array of [player_id, capture_date, decision_supported, payload_json-as-stored], sorted by player_id, UTF-8 / sort_keys / ensure_ascii=False / separators(",",":") / allow_nan=False) against `history_day_meta` inside ONE snapshot; mismatch, or missing meta post-effective, = fail-close. **Authority: CONVERGENCE-PENDING** — history-B after a pre-commit crash is flagged pending (meta.generation ≠ committed generation); evidence readers may use meta-verified content; surface consumers are unaffected (they read the committed pair); the next successful owning run for the SAME capture_date converges it.
...
- **guard_meta + ACTIVATION LATCH (r13-8, identity-anchored):** `effective_from_date` in committed config before any run. Bootstrap writes the db singleton FROM config plus a random `db_instance_id`. The latch (a second David-gated config commit after the first guarded commit) records `activated: true` + the activation `generation_id` + `db_instance_id`. Deterministic: PRE-latch db loss ⇒ re-bootstrap (new id, allowed); POST-latch: db absent, singleton missing, or `db_instance_id` mismatch ⇒ FAIL-CLOSE (recreation detected by IDENTITY); post-latch committed-marker loss ⇒ no_committed_generation + health alarm. Retirement = ONE merged sequence: first guarded commit → latch → scan green → retire unsuffixed → rescan. Backfill = pre-effective only.
