...
- **Reports are GENERATION-ADDRESSED** (r8: the shared-path report made row 16a false — a pre-gate degraded run would overwrite A's report bytes and break A's `report_sha256`). The report filename carries the run's id (`..._report_<generation_id>.json` for ok runs; `..._report_attempt_<uuid>.json` for degraded runs, which have no generation); the marker references its exact report path. Prior generations' reports are never touched; a retention rule (keep last N) is a build knob.
- **Report schema — the same two strict shapes:** ok-shaped = `generation_id` + both output hashes + `provisional=true`; degraded-shaped = nonempty `reason` + typed attempt metadata with generation/hashes explicitly null. The provisional flag is PHYSICALLY PERMANENT.
...
- **content_sha256, reproducible (pinned, FULL stored tuple):** SHA-256 over ONE enclosing JSON array of `[player_id, capture_date, decision_supported, payload_json]` — every stored column (r8 probe: an id+payload-only hash let a `decision_supported` flip pass). Sorted by `player_id`; canonical G2 encoding for the enclosing array. `payload_json` participates byte-identically AND is VALIDATED as JSON both before storage and on read with `parse_constant` rejecting NaN/Infinity tokens (r8 probe: enclosing-array `allow_nan=False` does not inspect an embedded string). Keys: exact normalized nonempty strings (no surrounding whitespace) required BEFORE the DELETE. Readers compute from ONE SQLite snapshot (meta + rows in a single transaction).
- **Reader contract:** live history readers verify the day's row count + content hash against `history_day_meta` WHEN a meta row exists; mismatch = fail-close. (Ref-pinned evidence additionally uses DG_MOVEMENT_HASHES.)
- **Legacy adoption (pinned; the live DB holds 2026-07-09/11/13 × 12,201 rows with NO meta):** days without meta are LEGACY — readable, flagged `pre-guard legacy` in reader output, protected only by evidence manifests. An optional one-time backfill may write meta rows with `generation_id=NULL, legacy=true` and hashes computed from current content — an honest checksum-of-what-exists, never a fabricated owner. The guard's effective date = its first owning run; it writes meta only for its own target day.
