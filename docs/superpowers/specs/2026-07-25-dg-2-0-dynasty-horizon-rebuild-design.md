...
**TICKET-SCOPE BOUNDARY RULE — RATIFIED AND BINDING (Ruling K, David 2026-07-25).** Every ticket in this epic is judged against it: *"A ticket states required observable outcomes and may name an internal technical restriction only when it cites a pre-existing, owner-ratified boundary and explains the consequence it protects; otherwise design, dependencies, tools, implementation sequence, and test method belong to the developer."* **Two ratified riders:** unresolved or contested cases **default to developer freedom**; process requirements (RED-first, independent review, second-lane reproduction) belong in a **program-wide Definition of Done**, never inside a ticket's acceptance criteria. **Full text, provenance and the falsifiable check: `docs/governance/rulings/2026-07-25-ticket-scope-boundary-rule.md`.** It is in-repo deliberately — **no ticket may be judged against a rule that lives outside the clone.** The rewrite pass against it is **not started and not authorised by the ratification**; it is Claude authoring with Codex reviewing.
