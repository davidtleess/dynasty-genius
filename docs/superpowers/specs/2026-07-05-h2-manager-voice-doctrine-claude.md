...
**Supersession note, 2026-07-08 (David tier-calibration correction):** the document's core permission still stands — calibrated quality words can be legal when word+number+receipt travel together and No-Verdict discipline holds. The fixed percentile ladder in §3 / §10, however, is **not** an implementation-ready threshold contract. David clarified that "Elite" must not be an arbitrary percentile; tier boundaries must be calibrated from relative model value, production, age/longevity context, and historical field separation. Treat the table below as the historical roadmap draft for lexicon names, not as the tier algorithm. Implementation must route through the Value Board v3.2 tier-calibration contract and a future `tier_calibration` artifact before any named labels ship.
