...
**Robustness boundary:** a malformed top-level `frozen_inputs` shape fails **loud** (caller/API misuse); a malformed *individual* source record (e.g. a roster row missing `id`) fails **closed** → `malformed_source_record` review row.
