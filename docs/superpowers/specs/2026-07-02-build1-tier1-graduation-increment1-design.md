  ...
  "surfaces": [
    {
      ...
      "gate_components": [
        ...
        { "component": "mif_breaker", "evidence": ["src/dynasty_genius/outcome_loop/realized_outcome_scorer.py", "tests/unit/test_realized_outcome_scorer.py"], "expectation": "wired; active-role deviation < 15% when evaluable; off-season = a presence PROBE returning insufficient_data honestly — never default-pass (Codex R4 + Gemini). REAL paths pinned at T4 GREEN (grep-verified model_input_fidelity): the kickoff draft guessed realized_outcome/scorer.py, which does not exist" },
