# Frontend Surface-3 Player Detail — Design Spec

**Status: v2 AMENDMENT (for cockpit dual-CLEAR) — v1 dual-CLEARED + committed `9a16e06`**
**Date:** 2026-06-06 (v2 amendment 2026-06-06) · **Author:** Claude Code · **Type:** Design spec — the build contract for Phase-12 frontend Surface 3 (Player Detail)

> **v2 amendment summary:** a round-2 cockpit audit (David-ruled) folds three issues — see **§8.2** and the new locked decisions **§0.1 items 5–7**. Live spec sections updated: §2.1, §2.2, §2.3, §3.1, §4, §6.

---

## 0. Scope & standing constraints

Surface 3 is **Player Detail** — instant, explainable evidence for one player (model grade, xVAR/DVS, market overlay, top drivers, risk flags, caveats, a steel-manned counter-argument) to support David's roster-cut and trade decisions. It is the **Decision-Evidence-Card contract applied to a single player**, and the reusable inspector substrate later surfaces consume.

**Binding inputs (do not relitigate):**
- **Frontend design spec** `docs/superpowers/specs/2026-06-03-frontend-design-spec.md` — §2 the three structural contracts (Decision-Evidence-Card, **Two-Lane**, Experimental); §3 invariants; §4 visual system (OKLCH tokens, **model = blue / market = amber**, **no verdict hues**, constitutional age cliffs amber); §5 app shell (left-rail, sticky Trust strip, ⌘K, **right-side collapsible player inspector**); §7 governance-in-UI (banned-language CI linter; `decision_supported=false` universal + non-dismissible).
- **Surface 1** (shell + Trust strip + ⌘K + Hey/Zod codegen seam + banned-language CI) and **Surface 2** (Trade Lab — two-lane panels, neutral divergence, honest degradation, asset catalog) are COMPLETE + MERGED on `main` (`2019424`/`df4b63d`). Surface 3 reuses their primitives.
- Constitution: market data is overlay-only and never a model input; `decision_supported=False` everywhere; banned David-facing patterns (rookie `confidence`/`dynasty_tier`, trade `verdict`, roster `action`) stay banned.
- Runtime-dep budget unchanged (React + ReactDOM + Zod); no TanStack/charting/router lib in v1; `localStorage`/ephemeral client state only.

**This spec is planning only.** No dependency install, no code. The build plan + cockpit TDD follow after this spec is committed.

### 0.1 Locked decisions (David, 2026-06-06)
1. **Form = hybrid** — the right-rail inspector is a **neutral selector/preview**; the full Decision-Evidence-Card is a **page** view.
2. **Scope = (b) active + prospects** — full card only for **modeled** rows (`ENGINE_A` ~80, `ENGINE_B` ~373, `BLEND_AB`); the ~11,700 non-modeled rows (`PRE_MODEL`/`INACTIVE`/`MARKET_ONLY`/`UNRESOLVED_IDENTITY`) return a typed **degraded Experimental** response — never a raw sparse card.
3. **Evidence sourcing = Option A** — extend `universe_pvo_batch.py` to preserve the assembler's evidence fields, rebuild the artifact.
4. **Market source = A-1** — read the precomputed `universe_market_divergence_latest.json` (market overlay + divergence), with a freshness caveat. Live-FantasyCalc is deferred.
5. **(v2) Issue-1 source rewrite pulled forward — with an audited regeneration.** The `counter_arguments.py` QB/TE templates that emit the banned standalone word "elite" are rewritten with a vocabulary-safe replacement ("Premium valuation" / "high-valuation"; NOT tier-family — no "elite"/"top-tier"/"premier"), preserving the same downside thesis (no analytic-meaning change). Because Engine-A rows are read **through** `prospect_cards.json` (not regenerated at batch time), the fix requires the **3-step regeneration** in §2.4 and an **integrity audit over BOTH `prospect_cards.json` and `universe_pvo_latest.json`** (model fields byte-stable; any drift = HARD STOP → escalate to David). The backend banned-term guard (§2.2) is RETAINED as silent defense-in-depth. (Supersedes the v1 "separate later task" deferral.)
6. **(v2) Issue-3 divergence styling locked — uniform-neutral.** The Player Detail divergence element is **uniform-neutral slate** (matching Surface-2 `DivergenceStrip.tsx:15`); direction is conveyed by **label text only** (e.g. weight/case — "Model HIGHER than Market"), never by directional brand color (no blue=higher / amber=lower). Blue/amber stay reserved for **lane identity** (model/market), not signal direction; red/green remain banned.
7. **(v2) Issue-2 inspector clarity.** The inspector preview labels **unmodeled categories explicitly** ("No active model score" / "Unmodeled category"), status-only and within the neutral-preview limits (never a mini-card), instead of a generic "evidence incomplete."
8. **(v2) Commit placement.** Spec patch + build plan commit to `main` (docs only); the **audited artifact regenerations** — `resources/prospect_cards.json`, its `resources/prospect_cards.js` wrapper (same payload), `app/data/valuation/universe_pvo_latest.json`, and the `docs/validation/phase15-2026-rookie-rank-refresh.md` report — all land on the **implementation/TDD branch**, never mixed into a spec/docs commit.

---

## 1. Architecture — two parts

Surface 3 = (A) a thin **backend evidence/endpoint layer** (Python; additive, read-only, `decision_supported=False`) + (B) the **Player Detail React surface** inside the existing AppShell.

```text
+------------------------------------------------------------------+
| AppShell (shell state: selectedSleeperId + inspector open/closed)|
|  PlayerInspector (rail: neutral preview)  --Open full detail-->  |
|  PlayerDetailPage (main: full Decision-Evidence-Card)            |
|    Header · ValuationTwoLane (model | market + divergence)       |
|    EvidenceSection (drivers · risk flags · caveats · counter-arg)|
+------------------------------------------------------------------+
| Generated Hey/Zod client (typed at the SDK boundary)            |
+------------------------------------------------------------------+
| FastAPI (read-only):  GET /api/players/{sleeper_id}  (NEW)      |
+------------------------------------------------------------------+
| Read-only artifacts:                                            |
|  universe_pvo_latest.json  (evidence — after Option A)          |
|  universe_market_divergence_latest.json  (market overlay + divg)|
+------------------------------------------------------------------+
```

---

## 2. Backend (Part A)

### 2.1 Option A — preserve the evidence fields
`pvo_assembler.assemble_pvo()` computes `counter_argument` (via `generate_counter_argument`), `risk_flags` (`_build_risk_flags`), `top_drivers`, and `caveats`, but `universe_pvo_batch.py` **drops them** during row serialization (verified: 0 occurrences of these keys in `universe_pvo_latest.json`).

- Extend the row-assembly loop in `universe_pvo_batch.py` to preserve **every DTO-backed source field the row currently drops** — the evidence fields `counter_argument`, `risk_flags`, `top_drivers`, `caveats` AND the identity/projection fields `draft_class`, `nfl_draft_pick`, `nfl_draft_round`, `projection_1y`, `projection_2y`, `projection_3y` (all verified `None` at row level in the current artifact (`universe_pvo_batch.py:88-100`, `:147-178`), but present on the source PVO — `player_value_object.py:68-80`, filled at `pvo_assembler.py:484-502`). Then **rebuild** the artifact. (`roster_audit` signals optional.)
- **Additive-shape contract guard:** the change is ADD-only. Every existing reader (`trade.py`/`trade_market.py` `_load_reconcile_artifacts`, `asset_catalog.py`, `roster_cut_engine.py`) reads specific keys via `.get()`, so new keys are ignored. A contract test MUST assert the existing Trade Lab + asset-catalog suites stay green against the rebuilt artifact, and that the previously-read keys are byte-identical.
- **(v2) Source-text cleanup precedes the rebuild — see §2.4.** The Issue-1 `counter_arguments.py` rewrite is applied, then `prospect_cards.json` is refreshed, then `universe_pvo` is rebuilt (the strict 3-step sequence). The new `counter_argument` key is not previously read, so the byte-identical guard on prior-read keys still holds; model fields (`dvs`/`xvar`/`*_class_rank`/`engine_*`) MUST be byte-stable across the regen (§2.4 audit).

### 2.2 New endpoint — `GET /api/players/{sleeper_id}`
A read-only endpoint on the (empty, unmounted) `players.py`, mounted in `main.py`. Returns a **curated typed `PlayerDetailResponse`** — never the raw PVO row.

```text
PlayerDetailResponse {
  sleeper_id: str
  identity: { full_name, position, team, age, draft_class, nfl_draft_pick, nfl_draft_round }
  engine_path: Literal["ENGINE_A","ENGINE_B","BLEND_AB", ... ]   # echoed
  model_status: Literal["modeled","experimental","unavailable"]
  model: PlayerModelLane | null      # null when not modeled
  market: PlayerMarketLane | null    # null when market overlay absent (degrades independently)
  evidence: PlayerEvidence | null    # null/partial when fields absent (auto-Experimental)
  caveats: list[str]
  source_timestamps: { pvo: str|null, market: str|null }
  decision_supported: false          # coercion-locked
}
PlayerModelLane { engine_used, model_grade, model_version, dvs, xvar, xvar_percentile_position, projections }
PlayerMarketLane { source: "fantasycalc", market_value, market_rank_overall, market_rank_position, divergence: { signal_label: model_higher_than_market|model_lower_than_market|inside_band|unavailable, delta }, source_timestamp }
PlayerEvidence { top_drivers: list[str], risk_flags: list[str], counter_argument: str|null, completeness: { drivers, risk_flags, counter_argument, market } }
```

**Source → DTO mapping (the endpoint MUST map artifact names to the DTO, not pass raw):**
`market_rank_overall` ← `market_overlay.overall_rank`; `market_rank_position` ← `market_overlay.position_rank`; `divergence.delta` ← `divergence.model_minus_market_delta` (`universe_market_divergence.py:54-64`, `:210-218`). `nfl_draft_pick`/`nfl_draft_round`/`draft_class`/`projection_*` ← the same-named preserved PVO row fields (§2.1).

Behavior:
- **Modeled row** (`ENGINE_A`/`ENGINE_B`/`BLEND_AB`) → the **full card *shell*** with **per-section completeness/degradation** (NOT guaranteed-complete content): `model` populated; `evidence` populated per the `completeness` flags (a section absent in the data renders Experimental, never fabricated — see the coverage counts in §2.3/§8); `market` from A-1 or degraded (§3).
- **Non-modeled row** → `model_status="experimental"`/`"unavailable"`, `model=null`, `evidence=null`, neutral message surfaced client-side ("No predictive model score active for this player category"); `xvar` null/unranked.
- **Engine-A rows render only the available objective evidence** — the assembler's `top_drivers` **explanation strings** + the preserved `nfl_draft_pick`/`nfl_draft_round`/`draft_class` + `dvs`/`xvar`. There is **NO promised typed raw-feature block**: the batch carries `top_drivers` as strings + draft capital + position-specific provenance (e.g. TE fields at `universe_pvo_batch.py:172-175`); it does NOT carry a typed dominator/breakout/YPRR/target-share feature set. (Engine-A `risk_flags` are also effectively empty — see §2.3 counts.) The endpoint MUST NOT emit rookie `confidence`, `dynasty_tier`, or bucket-% — verified absent from the source *fields*.
- **Banned-term runtime guard (backend) — the field-name check is NOT sufficient.** The preserved evidence **text** is David-facing output, and a source probe found the assembler's `counter_argument` strings literally contain the banned tier word **"Elite"** in 2/80 Engine-A rows (`13269`, `13330`; `counter_arguments.py:30-40`) even though the `dynasty_tier` field is absent — and the frontend banned-language linter scans *source*, not backend data, so it cannot catch this. The endpoint therefore **scans every emitted evidence string** (`counter_argument`, `top_drivers`, `caveats`) against the banned rookie/verdict vocabulary (the `banned_vocabulary.json` terms); any string containing a banned term is **suppressed and that element degraded to Experimental** with an `evidence_suppressed_banned_term` caveat — never emitted. **(v2) The guard is now defense-in-depth, not the primary fix:** the `counter_arguments.py` source text is rewritten in this work (§2.4) so production rows no longer carry the banned word; the guard remains to catch any future/other banned text. **Because the cleaned rows (`13269`/`13330`) no longer trip the guard, its contract test MUST inject a synthetic banned string (or mock a row)** — relying on the real rows would assert nothing. The unit test `tests/test_counter_arguments.py:59` (which pins the exact "Elite valuation assumes…" string) is updated to the new wording in the same change.
- `decision_supported` recursively coercion-locked False.

### 2.3 Market source (A-1) + a coverage audit
- The `market` lane reads `universe_market_divergence_latest.json` (it populates `market_overlay` + model-minus-market divergence; the PVO writes `market_overlay: null`), joined by **`sleeper_player_id`** (the PVO row key, `universe_pvo_batch.py:152`). The response carries the artifact's **source timestamp** + a `market_overlay_static_caveat` (price is batch-fresh, not live). Usable market delta exists on **62/80 Engine-A and 211/373 Engine-B** modeled rows; rows without it render the market track degraded (§3.2).
- **Coverage counts (measured at source — drive per-section degradation, not a promise of completeness):** after Option-A preservation — **Engine-A (80):** `counter_argument` 8/80, `risk_flags` 0/80, `top_drivers` 80/80, `caveats` 80/80; **Engine-B (373):** `counter_argument` 244/373, `risk_flags` 181/373, `top_drivers` 373/373, `caveats` 373/373. So **the "full card" is a shell**: prospect cards are mostly drivers + caveats + draft capital (counter-argument ~10%, risk-flags ~0%); active cards add a counter-argument ~65% / risk-flags ~49% of the time. Each absent element renders Experimental via the `completeness` flags; the endpoint **never fabricates text**.
- **Artifact loading:** both artifacts are large (~20 MB PVO + ~24 MB divergence); the endpoint loads them once and caches (module-level, like the Trade Lab `_load_reconcile_artifacts`), not per-request. (Caching mechanics are a plan detail.)

### 2.4 (v2) Issue-1 source rewrite + audited regeneration

**Why a multi-step regen.** Engine-A prospect rows are read **through** `prospect_cards.json` (`build_universe_pvo_batch.py:25/:35/:130` → `_load_prospect_pvos`); `assemble_pvo`/`generate_counter_argument` runs only on the Engine-B active assembly span (`scripts/build_universe_pvo_batch.py:78-120`, `_active_pvos_from_engine_b`). So the offending Engine-A rows (`13269`/`13330`) carry the "Elite" text **from the stale `prospect_cards.json`**, not from regenerating the template at batch time. Rewriting `counter_arguments.py` + rebuilding `universe_pvo` alone would re-serialize the old text. (`prospect_cards.json` is tracked and *does* contain the verbatim string — verified.)

**3-step regeneration sequence (strict order):**
1. Rewrite `counter_arguments.py` — **QB `:33` "Elite valuation" → "Premium valuation"; TE `:39` "elite status" → "premium status"** (vocabulary-safe; no tier-family — no "elite"/"top-tier"/"premier"), **preserving the same downside thesis** (no analytic-meaning change). Update `tests/test_counter_arguments.py:59`.
2. Run `scripts/refresh_prospect_cards.py` → regenerates **three** tracked outputs: `resources/prospect_cards.json` (its `counter_argument` via `assemble_pvo`, `:126`), `resources/prospect_cards.js` (the Rookie Board JS wrapper of the same payload, `:393-396`), and `docs/validation/phase15-2026-rookie-rank-refresh.md` (the rank-refresh report, `:343`).
3. Run `scripts/build_universe_pvo_batch.py` → rebuilds `app/data/valuation/universe_pvo_latest.json` (also applies the Option-A §2.1 preservation).

**Integrity audit (a dedicated script; a pre-commit gate on the TDD branch).** Both `prospect_cards.json` (~7.5k lines) AND `universe_pvo_latest.json` (12,189 rows) are tracked artifacts with downstream consumers — `prospect_cards.json` feeds `draft_pick_valuation.py` (Regime-A pick pricing reads `xvar_class_rank`), `mfl_rookie_adp_divergence.py`, and `calibrate_sf_qb_knob.py`; `universe_pvo` feeds the Trade Lab / asset-catalog readers. The audit compares pre/post and asserts, **per artifact, an allowlist-complement contract: every existing JSON key/path is byte-identical pre→post EXCEPT the explicit permitted-changes allowlist below.** Inverting the contract (allowlist-complement, not an enumerated protect-list) is deliberate — it cannot silently miss a model field. The protected set therefore includes, **non-exhaustively**, `dynasty_value_score`, `model_grade`, `model_version`, `valuation_status`, `signal_completeness`, `rank_delta`, `value_above_replacement`, `dvs*`, `xvar*`, `*_class_rank`, `engine_used`/`engine_path`, `projection_1y/2y/3y`, market fields, `decision_supported`, and the Engine-A inputs `final_college_age`/`te_ryptpa_final`/`te_yards_per_reception_career` — but the **GATE is the allowlist complement, not this list**.

- **Key-set closure (no new-key escape):** the byte-identical check only covers pre-existing paths, so it is paired with a key-set assertion — each post-regen artifact's full set of keys/paths MUST equal its pre-regen key-set **plus exactly the allowlisted new keys** (the 10 §2.1 keys for `universe_pvo_latest.json`; **none** for `prospect_cards.json` / `.js`). Any other new, un-allowlisted key appearing only in the post-regen file is a **HARD STOP**.
- **Identity (per artifact):** `prospect_cards.json` row set identical on **`sleeper_id` + `player_id`**; `universe_pvo_latest.json` row set identical on **`sleeper_player_id` + `dg_player_id`**.
- **Permitted-changes allowlist (exact paths; everything else byte-identical):**
  - `prospect_cards.json` (+ its `.js` mirror): `counter_argument` **only on the cleaned QB/TE rows** (`13269`/`13330`); `assembled_at`.
  - `universe_pvo_latest.json`: top-level `captured_at`; row-level `captured_at`; `pipeline_run_id`; and the **10 newly-preserved keys** (§2.1).
  - `resources/prospect_cards.js`: its embedded payload must **deserialize equal to** `prospect_cards.json` (the same allowlist applies transitively).
  - `docs/validation/phase15-2026-rookie-rank-refresh.md`: report timestamp + cleaned-row lines only.
- **Explicitly STABLE (drift → STOP):** `source_versions`, `source_season`, `source_snapshot_captured_at`, and `lineage` hashes are input/model provenance, NOT run churn — any movement triggers escalation.
- a pre/post banned-vocabulary scan of emitted evidence text on all regenerated artifacts → **zero** banned standalone words post-regen.
- **Any change outside the allowlist — model-field, ranking, OR input-provenance drift — is a HARD STOP → escalate to David; no auto-proceed.** (Such drift would indicate feature/model/data movement since the artifacts were last built and would ripple into Regime-A / MFL / SF-QB.)

**Commit placement.** This subsection's source rewrite + both regenerated artifacts + the audit script land on the **implementation/TDD branch**, never in the spec/docs commit on `main`.

---

## 3. Frontend (Part B) — the hybrid

### 3.1 Components
- **`PlayerInspector`** (right rail) — the neutral **selector/preview**. Renders ONLY: identity (name/position/team/age/draft capital), `model_status` + market availability, and a **neutral presence indicator** — a plain count: `"3 caveats · counter-argument available"`. A primary **"Open full evidence card"** action. **Never** renders: a grade, edge, model-vs-market delta, recommendation, truncated counter-argument/caveat text, a subjective tier, or a warning glyph. It carries the universal `decision_supported=false` state ("Preview only — open full detail for evidence"). **(v2)** Missing/unmodeled evidence → an **explicit category label** ("No active model score" / "Unmodeled category" / "Evidence incomplete — open full detail"), status-only and within the neutral-preview limits (no scores, no decision language, never a mini-card), so David sees the category without a navigation click.
- **`PlayerDetailPage`** (main view) — fetches `GET /api/players/{sleeper_id}`, validates at the Zod boundary, renders the full card or the degraded Experimental state.
- **`PlayerDetailCard`** — header (name/age/team/position/draft class+pick) + the sections below.
- **`ValuationTwoLane`** — **model track** (blue: engine_used, model_grade, xVAR/DVS, position percentile, projections) | **market track** (amber: FantasyCalc value, overall/position rank, source timestamp + static caveat) — physically separate, never blended. A **uniform-neutral (slate)** **divergence** element (`model_higher_than_market` / `inside_band` / `unavailable`), **descriptive only** (divergence remains unvalidated — never a buy/sell verdict). **(v2)** Direction is conveyed by **label text only** (e.g. weight/case — "Model HIGHER than Market"), **never by directional brand color** (no blue=model-higher / amber=market-higher); blue/amber stay reserved for **lane identity** (model/market), matching Surface-2's neutral `DivergenceStrip.tsx:15`. Red/green banned.
- **`EvidenceSection`** — top drivers, risk flags (objective; constitutional age-cliffs amber), caveats list, and the **full** steel-manned counter-argument (no truncation).
- **Shell state:** a single `selectedSleeperId` + inspector open/closed lives in AppShell state; any surface (the asset search, Trade Lab chips) may set it. v1 entry points: the asset-catalog search + (light) Trade Lab chip → inspect. No router/URL deep-linking in v1.
- **Reused from Surface 1/2:** AppShell, tokens, the universal non-dismissible `decision_supported=false` banner, the Experimental treatment, the asset-catalog search, the Zod-boundary fetch + degradation pattern.

### 3.2 Honest degradation
- **Non-modeled player** → the Experimental/unavailable treatment (both inspector + page), neutral message, no sparse card.
- **Per-element degradation:** a modeled player missing `counter_argument` (e.g. `generate_counter_argument` returns null for DVS≤80 with no risk flags) → that section shows an explicit "no counter-argument generated" Experimental state, not fabricated text. The `completeness` flags drive each section.
- **Market track degrades independently** (absent/stale `market_overlay`) without implying the model track is decision-grade.
- `decision_supported=false` universal + non-dismissible across the surface.

## 4. Visual system
Reuse the Surface-1/2 OKLCH tokens unchanged. **Model = blue, market = amber**, physically separate; **no green/red verdict hues**; constitutional age-cliff amber on the relevant risk flags. Uncertainty stays prominent; the counter-argument is a first-class, non-collapsed section. The inspector preview is deliberately sparse (identity + status + presence counts). **(v2)** The divergence element is **uniform-neutral slate** — no directional brand color; direction reads from the label text only (blue/amber denote lane identity, not signal direction), consistent with the Surface-2 neutral DivergenceStrip.

## 5. Governance
- **Engine-A objective-features-only** — no rookie `confidence`/`dynasty_tier`/bucket-% (banned), enforced at the endpoint (field absence **+ the backend banned-term scan of evidence *text*, §2.2** — closes the gap that the frontend linter, which scans source not backend data, cannot) and the UI (banned-language linter + a render guard).
- The **banned-language CI linter** (existing, walks `src/`) covers the new components.
- **Two-lane separation** by component structure; **neutral divergence** only; the inspector preview cannot imply a verdict by omission (presence counts only).
- Mandatory counter-argument + caveats (the Decision-Evidence-Card contract); `decision_supported=false` universal.

## 6. Testing (cockpit TDD: Codex RED → Claude GREEN → dual-CLEAR)
- **Backend:** Option-A additive preservation — **all DTO-backed fields present** (`counter_argument`/`risk_flags`/`top_drivers`/`caveats` AND `draft_class`/`nfl_draft_pick`/`nfl_draft_round`/`projection_1y-3y`, not just evidence); the prior-read PVO keys byte-identical; Trade Lab + asset-catalog suites green against the rebuilt artifact. Endpoint contract (modeled→full-shell `PlayerDetailResponse`; non-modeled→typed degraded; raw PVO shielded; Engine-A emits no banned *fields*; **(v2) the backend guard suppresses + degrades a SYNTHETIC injected banned-term evidence string (NOT real rows — the §2.4 rewrite cleans `13269`/`13330`)**; market from A-1 with freshness caveat + source→DTO mapping; `decision_supported` recursively False; missing element→completeness-flag, no fabrication).
- **(v2) Source rewrite + regen audit:** `tests/test_counter_arguments.py:59` asserts the new vocab-safe wording (no banned standalone word, no tier-family); the integrity-audit script (§2.4) over ALL regenerated artifacts (`prospect_cards.json` + `.js` mirror + `universe_pvo_latest.json` + the phase15 report) enforces the **allowlist-complement** contract (every path byte-identical except the per-artifact allowlist; `.js` payload deserializes equal to `.json`; `source_versions`/`source_season`/`source_snapshot_captured_at`/`lineage` stable), with **any out-of-allowlist drift = fail → STOP**, and a post-regen banned-vocabulary scan returns zero.
- **Frontend (Vitest):** inspector renders identity + status + presence counts ONLY (no grade/edge/delta/truncated-text/glyph/recommendation); full card renders the two physically-separate tracks + the full counter-argument + caveats; neutral divergence (no verdict); non-modeled→Experimental; market track degrades independently; per-element auto-Experimental on null counter-argument; `decision_supported` banner present + non-dismissible; **(v2)** the inspector labels unmodeled categories explicitly (status-only, no scores/decision language, not a mini-card); the divergence element is uniform-neutral slate (no directional-color class), direction via label text only.
- **Gates:** `tsc`/Biome/Vitest/build; full Python suite; S4 byte-audit run as a gate (the new endpoint + batch change touch no S4 inviolate path — confirm, don't assume).

## 7. Out of scope (v1)
Live-FantasyCalc market source (deferred — A-2); deep-link/URL routing for the page; cross-surface "selected player" beyond the single shell `selectedSleeperId`; a separate Roster-Audit entry point (Roster Audit is a future surface); any analytical/model/feature change.

---

## 8. Resolved (cockpit, 2026-06-06 — pre-spec brainstorm input)
Surface selection + form/scope/sourcing were shaped via a David-driven brainstorm with cockpit input (Codex technical + Gemini governance), both source-verifying the artifacts. Resolutions, all locked above:
1. **Player Detail first** (lowest-risk, gate-free, foundational substrate) — three-way concur.
2. **Hybrid form**; inspector = neutral selector/preview (Codex: not a compact card; Gemini: presence-indicator) — David ruled **neutral presence indicator** (plain counts, no warning glyph).
3. **Scope (b)** active + prospects; modeled-only full card; non-modeled→degraded; Engine-A objective-only.
4. **Option A** evidence sourcing (additive, contract-checked) + **per-engine completeness audit**.
5. **Market = A-1** (divergence artifact + freshness caveat); live-FC deferred.
6. Verified: `counter_argument`/`risk_flags`/`top_drivers` absent from the PVO (Option A adds them); `market_overlay` null in PVO but populated in `universe_market_divergence_latest.json`.

### 8.1 Cockpit pressure-test round 1 (2026-06-06) — defects folded in
Codex + Gemini source-verified the spec; 8 findings (dedup ~5), all contract-precision/data-promise gaps (no blockers); all folded above:
- **Option A scope widened** — preserve `draft_class`/`nfl_draft_pick`/`nfl_draft_round`/`projection_1y-3y` too (not just evidence fields), or the DTO returns null (§2.1).
- **Naming** — `draft_capital`→`nfl_draft_pick`+`nfl_draft_round`; join key `sleeper_player_id`; market source→DTO mapping table (`market_overlay.overall_rank`/`position_rank`, `divergence.model_minus_market_delta`) (§2.2/§2.3).
- **Engine-A claim weakened** — available `top_drivers` strings + draft capital, NOT a typed raw-feature block (the batch lacks dominator/YPRR/target-share fields) (§2.2).
- **"Full card" reframed to a shell with per-section completeness/degradation** + the measured coverage counts recorded (§2.3): the evidence is honestly **thin, especially prospects** (Engine-A counter-argument 8/80, risk-flags 0/80). David ruled scope (b) stands — degradation handles it transparently; active players (the decision-bulk) are the better-covered cohort.
- **Verified by the cockpit:** additive-shape safe (all PVO readers `.get()`-only); Engine-A source carries no banned tier/confidence **fields**, but the evidence **text** can contain banned terms (the word "Elite" in rows `13269`/`13330`) — now closed by the §2.2 backend banned-term suppression guard **(v2: superseded — the source text is now rewritten too; see §8.2 + §2.4)**; S4 byte-audit unaffected (`universe_pvo_batch.py` not byte-locked; `app/api/routes` outside the **S4 AST scan roots** — this clause refers to the S4 byte/AST audit ONLY, not the banned-language validation, which DOES cover the new components per §5).

### 8.2 Cockpit pressure-test round 2 (2026-06-06, v2 amendment) — David-ruled
A second adversarial round — David requested the whole team's genuine opinion; both reviewers returned evidence-bound objections (not CONCURs), and Claude verified each against the code (correcting one of its own earlier errors). Folded above:
1. **Issue 1 — "Elite" suppressor irony (Gemini-raised, all verified):** the guard would suppress + degrade the steel-manned counter-arguments of the highest-value QB/TE assets (`elite` is a banned standalone word, `banned_vocabulary.json:36`; emitted by `counter_arguments.py:33`/`:39`). **David ruling: pull the rewrite forward with a full audit harness.** Corrections folded: the regen target is `universe_pvo` **and** `prospect_cards.json` (Claude's earlier "prospect_cards has no elite" was WRONG — it carries the verbatim string; miss logged); the Engine-A read-through (`build_universe_pvo_batch.py:25/:35/:130`) requires the **3-step regen** (§2.4); vocab "Premium"/"high-valuation" (avoid top-tier/premier); `tests/test_counter_arguments.py:59` updated; guard test → synthetic string.
2. **Issue 5 — blast radius (Claude-raised; Gemini + Codex CONCUR):** `prospect_cards.json` has 7 consumers incl. Regime-A pricing / MFL / SF-QB → the integrity audit covers BOTH artifacts with byte-stable model fields; **drift = HARD STOP → escalate to David**.
3. **Commit placement (Codex-raised; Gemini CONCUR):** the artifact regens land on the TDD branch, not the main docs commit.
4. **Issue 2 — inspector clarity:** explicit unmodeled-category labels (neutral limits).
5. **Issue 3 — divergence neutrality:** uniform-neutral slate, direction via label text, no directional brand color (matches `DivergenceStrip.tsx:15`); blue/amber = lane identity only.

### 8.3 Cockpit pressure-test round 3 (2026-06-06) — v1-patch review, defects folded into v2
Codex + Gemini adversarially reviewed the v1 patch (both REJECT with concrete defects); all verified and folded:
1. **Audit contract inverted to allowlist-complement** (Codex; Claude had converged on the same) — the enumerated model-field list could silently miss a field (`dynasty_value_score`/`model_grade`/`model_version`/`valuation_status`/`signal_completeness`/`rank_delta`/`value_above_replacement`/projections/Engine-A inputs). v2 asserts every path byte-identical except a per-artifact allowlist (§2.4).
2. **Provenance allowlist tightened** (Codex + Gemini) — `source_*` was too broad; v2 allows only `captured_at`/`assembled_at`/`pipeline_run_id` and pins `source_versions`/`source_season`/`source_snapshot_captured_at`/`lineage` STABLE (drift→STOP).
3. **Per-artifact identity keys** (Codex + Gemini) — `prospect_cards` = `sleeper_id`+`player_id`; `universe_pvo` = `sleeper_player_id`+`dg_player_id` (was a wrong shared shape).
4. **Side-effect artifacts named** (Gemini, verified) — `refresh_prospect_cards.py` also writes `resources/prospect_cards.js` (`:393-396`) + `docs/validation/phase15-2026-rookie-rank-refresh.md` (`:343`); both added to §2.4 / §0.1-item-8; `.js` payload must equal `.json`.
5. **Stale references fixed** (Codex) — `player_value_object.py` casing; Engine-B span `:78-120`; the AST-scan-roots clause clarified to the S4 audit only (not banned-language validation).
6. **Key-set closure** (Gemini, v2 review) — the allowlist-complement byte-check only covers pre-existing paths, so it is paired with a key-set assertion (post key-set = pre + allowlisted new keys only) to stop a NEW un-allowlisted key escaping (§2.4).
Vocab map locked: QB "Elite valuation"→"Premium valuation"; TE "elite status"→"premium status".

---

**Next after dual-CLEAR:** a build plan (`docs/superpowers/plans/2026-06-06-frontend-surface-3-player-detail.md`) — both committed to `main` (docs-only) and pushed to `origin/main` — then cockpit TDD on a branch cut from `main`; the §2.4 source rewrite + audited artifact regens live on that branch, never in the docs commits. No dependency install, batch rebuild, or code until this spec patch is committed.
