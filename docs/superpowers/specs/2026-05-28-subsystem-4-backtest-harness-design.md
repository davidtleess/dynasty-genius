...
#### 11.2b — Backtest-A input-readiness preflight (Amendment 2026-06-01, David-approved; cockpit dual-CLEARED)

A **diagnostic-only** preflight that answers, *before* a live run, whether the real-mode Backtest-A inputs are present / valid / aligned / free of the statically-determinable §11.2 hard-blocks — naming exactly what is missing. It is the acceptance gate for an operational run and gives feedback while the S3 confirmed-class fixtures are curated. **Strictly descriptive / diagnostic** — it does NOT validate model predictions, market divergence, or represent decision-grade clearance; `decision_supported` is untouched and the report carries no decision-grade field.

**Component.** `preflight_backtest_a_inputs(snapshots_dir, identity_dir, draft_year, *, override_draft_date=None, include_untrusted=False, run_id=None, output_root=None) -> BacktestAPreflightReport` (mirrors `run_backtest_a`'s `include_untrusted` so the dry ingest reflects the same trust posture the real run would use) — in `eval/backtest_mock_draft.py` (an existing allowlisted eval file; avoids the `AUTHORIZED_EVAL_FILES` landmine) + a thin CLI `scripts/preflight_backtest_a.py`. Reuses `_find_bridge_artifact` / `load_bridge` / `load_registry` / `ingest_snapshots` / `_confirmed_class_selection_bias`. **Read-only: no aggregate / join / metrics / artifact write.** Checks **accumulate** (collect all issues in one pass), not fail-fast.

**Checks (each → `ok` / `blocked` / `not_checked` with a detail string):**
1. **Explicit presence (missing AND zero-byte).** `snapshots_dir` exists + contains ≥1 snapshot file; the identity bridge artifact (`_find_bridge_artifact`) exists + non-zero-byte; `college_prospect_registry.json` exists + non-zero-byte — checked via `Path.exists()` + size **before** the loaders (which no-op on missing → cannot be relied on to detect absence).
2. **Dry read-only ingest + schema scan.** (a) A per-file read-only **schema scan** — `MockSnapshot.model_validate(json.loads(<file>))` for each snapshot file — yields the `schema_invalid` count (ingest *silently skips* schema-invalid snapshots and does NOT surface a count, so this separate scan is the source of `schema_invalid`). (b) A read-only `ingest_snapshots(..., include_untrusted=…)` yields the surviving `normalized_picks` count and the resolved draft date. **`blocked` if zero surviving normalized picks** (a non-zero file count can still yield zero usable picks). **`blocked` if no draft-date resolves** — pinned to the ingest sentinel: `coverage["draft_date_source"] == "no_snapshots"` (equivalently `draft_date_used == "9999-12-31"`), unless an `override_draft_date` is supplied.
3. **Loadability + draft-year alignment.** Registry + bridge load; bridge `metadata.draft_year` matches `draft_year`; the registry has ≥1 `confirmed` entry for `draft_year`.
4. **Static §11.2 coverage.** `_confirmed_class_selection_bias` → `confirmed_class_unbridged_count` + sorted `confirmed_class_unbridged_uuids` + `orphan_bridges_detected`; **`blocked` if `confirmed_class_unbridged_count > 0` or any orphan**. `consensus_unbridged_count` is NOT computed here — it requires aggregate+join and is **run-time-only** (the report states this).
5. **Output collision.** `not_checked` unless **both** `run_id` and `output_root` are provided; if both given, `blocked` when the run's `backtest_a_result.json` path already exists (the duplicate-run_id refusal `run_backtest_a` enforces).
6. **Live truth-source.** Always `not_checked` — the live nflreadpy draft-truth fetch is NOT probed (it fails loud at run-time per §4/§4a). A `ready` preflight therefore means *input-ready excluding the live truth fetch* — explicitly NOT a guarantee the live run's truth load will succeed.

**Output.** A typed `BacktestAPreflightReport` (`ready: bool`, `checks: list[{name, status, detail}]`, `blocking_reasons: list[str]`, the §11.2 coverage counts, an ingest summary `{snapshot_files, schema_invalid, normalized_picks, draft_date_resolved}` (`schema_invalid` from the per-file scan; `normalized_picks` the survivors from the dry ingest)). `ready` is True iff no check is `blocked`. **No `verdict`/`pass`/decision-grade wording** — `ready` is input-readiness only.

**CLI (`scripts/preflight_backtest_a.py`).** Prints the verbatim header `DESCRIPTIVE / DIAGNOSTIC — not decision-grade. No edge claim.` and the disclaimer `Preflight checks INPUT READINESS only (file presence, schema validation, and selection-bias gate prerequisites). It does NOT validate model predictions, market divergence, or represent decision-grade clearance.`, then the report; exits `0` when `ready`, non-zero when blocked.
