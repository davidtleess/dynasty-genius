...
How the in-season loop is *allowed* to move is now a Locked Analytical Ruling — `00-product-constitution.md` → "In-Season Estimate Responsiveness And Model-Change Governance" (David-ratified 2026-06-27): daily **estimates** update via a deterministic, versioned overlay on a **frozen model artifact** (lead on usage/role, lag on noisy outcomes, position-varying responsiveness, **bias to stability**); the **model itself never auto-adjusts in-season** (monitors detect/report/queue; promotion is human-gated + pre-registered). This governs roadmap items #1 (capture the frozen-model vintages), #3 (Trust Console grades the frozen model, not the overlay), and #5 (trend overlays stay descriptive).
