...
- **Scoped** reverse scan (Codex R5): only served roots + pointer-resolved run dirs, never a blanket `app/data/models/**` walk, never `tests/` fixtures; `.pkl` not in the registry → `unregistered_local`; unreferenced by any governing pointer (off-path OR an unreferenced sibling inside a referenced run dir) → strictly `info`, never degrades `overall_status` (severity keys on pointer-REFERENCED file paths, not directory containment — clarified 2026-07-02, spec §3.5).
- **RED (Codex):** seed 9 (stray unreferenced → info, does not degrade overall), seed 10 (pointer-referenced-but-unregistered path, production → integrity/blocked; unreferenced sibling → info), seed 18 (`load_verification_status: not_verified`), **seeds 19–21 (pointer health: hash-ok pkl + missing/malformed/mismatched governing pointer must NOT yield overall `ok` for active+required serving; `not_applicable` when no governing_pointer)**; missing-manifest handled gracefully via temp dirs; assert the scan does not walk historical run pkls or test fixtures.
