...
- label identity: on joined rows, `appear_j == appeared_year1`, `|points_j − points_year1| <= LABEL_ATOL` with `LABEL_ATOL = 1e-9` and NO relative tolerance (root finding: `np.isclose(atol=1e-6)` keeps rtol 1e-5 and would pass 250 vs 250.002 while claiming identical labels; the frozen labels are exactly equal), `games_j == games_year1`; rows with NaN on the rookie label side are NOT joined rows (they are ledger rows, Task 4); a NaN on one side only is a mismatch and raises.
- veteran year arithmetic is ASSERTED on the veteran's own rows, not recomputed: `forecast_season == feature_season + 1 == draft_season + experience` for every joined row, else raise (a corrupted `forecast_season` with equal labels must refuse).
- rookie rows whose `forecast_year != draft_season` are never filtered silently: `rookie_draft_time_frame` raises naming the count (the frozen file has none; if a future writer emits multi-year rows the audit must be told explicitly, not guess). The draft-time frame's `(gsis_id, draft_season, pick, position)` must agree with `cohort.csv`, else raise.
- a joined row without a basic-cohort row (`veteran_games_t` NaN) raises: a historical prediction without its feature row is a broken input, and `thin_history` must never default to False from a missing key.
...
- Produces:
  ...
  - `write_audit(run_dir: Path, result: dict, *, rookie, veteran, seed, draws, experiences, git_sha: str) -> dict` writing `joined_rows.csv`, `coverage_ledger.csv`, `veteran_population_ledger.csv`, `metrics.json`, `REPORT.md`, `manifest.json`; returns the manifest. `manifest["outputs_sha256"]` covers every written file except itself. **Root acceptance notes (2026-09-06):** (a) the WRITER refuses when any of its output files already exists in `run_dir` (`FileExistsError`) — tested by calling `write_audit` twice on the same directory, not by testing `create_run_dir` alone; (b) `manifest["inputs"][side]["manifest_sha256"]` records the sha256 of each input run's `manifest.json` bytes as parsed, beside the consumed CSV/parquet hashes, so a replay can prove it read the same declarations; (c) `metrics["definitions"]["caveats"]` carries verbatim: the rookie manifest's own evidence caveat (policy menu refined after inspecting the historical years — read from `rookie.manifest["evidence_status"]` when present), the bootstrap conditioning (fixed forecasts and realized seasons; no model-selection or season uncertainty), the role caveat, and the window distinction (a veteran feature row with games outside the championship window but no window appearance is VALID evidence, flagged not rejected — 14 such pairs in the frozen data, e.g. final-week or postseason-only stat lines).
