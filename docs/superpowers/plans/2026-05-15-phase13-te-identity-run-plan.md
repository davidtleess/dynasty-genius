...
| No `pff_te_eligible` manifest writer | Medium | Add a post-gate step that filters rows to `identity_materialization_gate.RESOLVED_STATUSES` (`RESOLVED_DETERMINISTIC`, `RESOLVED_MANUAL`) with canonical `player_id`, then writes the eligibility manifest. |
