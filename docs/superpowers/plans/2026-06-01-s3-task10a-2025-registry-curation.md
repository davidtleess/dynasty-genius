...
- [ ] **Step 1 (RED, Codex):** assert `build_2025_prospect_fixture(frozen_inputs) -> (rows, review_queue)`:
  ...
  - **robustness boundary (D4):** malformed top-level `frozen_inputs` shape → fails **loud** (caller/API misuse); a malformed individual source record (e.g. roster row missing `id`) → fails **closed** → `malformed_source_record` review row.
