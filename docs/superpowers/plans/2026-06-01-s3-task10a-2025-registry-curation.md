...
- [ ] **Step 1:** run Task 1 freeze against the real CFBD key (David-gated) with **`roster_year=2024, draft_year=2025`** (spec §2 year decoupling) → `_frozen_2025/`. (The 2026-06-02 run with a single `year=2025` produced a 0-match fixture — superseded; the `cfbd_roster` artifact carries the 2024 roster.)
- [ ] **Step 2:** run Task 2 builder → `resources/prospect_fixtures/2025_fantasy_prospects.json` + a review/block queue of unmatched/ambiguous rows. UDFA layer added with two-source corroboration (official team release outranks aggregators). **Step-2 decouple-TDD precondition (2026-06-02 year fix):** the fixture-build runner `scripts/build_2025_fixture_from_frozen.py` must load the CFBD roster artifact by its actual **`cfbd_roster_{roster_year}`** name (= `cfbd_roster_2024` for this class — via manifest/stem or a `cfbd_roster_*.json` glob, **not** the hardcoded `cfbd_roster_2025.json`) before re-running T4, while still writing only `2025_fantasy_prospects.json` + `2025_review_queue.json`. (Per spec §2 year-decoupling; covered alongside the freeze core + T3 runner in the Step-2 decouple TDD.)
