...
## Root's frozen amendments (definitions v2; v1 kept as the superseded proposal; no result inspected under either)

1. **Primary cohort = drafted early-career not-yet-contributor screen:** verified draft class c ≤ t, 1 ≤ t − c + 1 ≤ 3, and no season in c..t with (appeared AND DG-179 points ≥ the primary bar of that season). Resolved absent row = convention zero. The observed-history ≤ 3 stratum is a separate exploratory cohort and is never called NFL experience. No contemporary roster eligibility.
2. **Bars:** primary QB 37 / RB 45 / WR 71 / TE 21 (deep-roster relevance); strict sensitivity QB 24 / RB 36 / WR 48 / TE 12 (positional-depth scenario). Bars come from the full positional outcome panel of each season, never from candidates. Contributor = appeared AND points ≥ bar. The cohort is fixed under the primary bar while the label bar varies.
3. **Primary test = summed future:** ranking `policy_e_points_year2 + policy_e_points_year3`, required finite for both (missing forecast origins are counted exclusions), against realized t+2 + t+3 points and any-contributor, on identical complete candidates; comparators: origin actual points, frozen year-1 expected points (help now), draft pick, persistence comparator (`baseline_e_points` = total_points_t × training retention × appearance rate; it orders like all-games production inside a cell). Year 1 is a separate immediate-help analysis; years 2–5 separately are exploratory, year 5 being origin 2020 only.
4. **Budgets:** 2 per position per origin primary; 1 and 3 sensitivity; hits, misses, zero-appearance busts, realized future points per slot; paired player-cluster differences; per-origin results.
5. **No null constant:** the baseline arm discriminates; it is a labelled persistence comparator (`rank_persistence`), never asserted constant.
