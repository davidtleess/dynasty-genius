...
| `source_rank` | required | Source hierarchy rank: 1=ground truth, 2=validated analyst, 3=market signal |
...
```json
{
  ...
  "compliance_header": "Compliance: 100% / 0% (PASS)",
...
| `compliance_header` | required | Generated by `app/utils/compliance.py::calculate_compliance_ratio` |
  ...
  "players": [
    {
      ...
      "asset_management_signal": "LIQUIDATE | ELITE_HOLD | null",
      "signal_drivers": [
...
- `elite_exception_hold` is a caveated age-curve exception, not an action. It is allowed only for an RB at/after the age cliff when verified yards after contact per attempt is at least `3.0`. In that case, `asset_management_signal` MUST flip from `LIQUIDATE` to `ELITE_HOLD`.
...
- 2027 1st-round picks carry the strategic generational-anchor floor from `docs/2027_CLASS_TRACKER.md` and must not be valued below 120 DVU.
...
- Every top-level response and per-asset record MUST include `compliance_header`.
