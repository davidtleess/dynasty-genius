# Phase A framing v8 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v7 (`5e883be1…`). Responsive to the
Codex round-7 review: five findings, **ACCEPTED 5/5, zero contested**. **Framing only — no RED,
...
4. **before the receipt transaction, the NEWLY PUBLISHED object passes the SAME descriptor-bound
   invariant as the reuse branch** (round-7 R7-2): regular file, **`st_nlink == 1`**, size, full
   hash — holding and comparing the staged/published **inode**, not revalidating a pathname. A
   hard link planted on the staged inode before publication makes `st_nlink == 2` and refuses;
5. **Commit the offering receipt LAST**, one SQLite transaction, uniqueness on `receipt_id` and
...
publication then mutate through the alias (no receipt, no altered canonical object).
...
**Positive vector — grounded in the REAL input** (adp `1f7afcbf…`/30,388 B · sidecar
`25be2d5a…`/260,688 B · archive `d8af09851ec1e4d2df20d91940def997206f9d698671fed397fd27234772a54c`/
8,540,590 B · `offering_id=fbg-offering-2026-08-05-a` · declared retrieval
`2026-08-05T20:57:00-04:00` → canonical `2026-08-06T00:57:00Z`):
...
| N2 | **assignment swap under the fixed role order**: `role=adp;sha256=25be2d5a…;bytes=260688` then `role=identity_sidecar;sha256=1f7afcbf…;bytes=30388` | `content_vintage_id` = `fb6b16f63985abf2efd72b1d311217bcb8cc151c9dc58f57dfb7b8bbc6f1d86f` |
...
| 4 | valid, <30d | `review_required` (same) | **none** | `current` | `Last … N days ago · latest drop awaiting data review` | 0 |
| 5 | valid, <30d (older offering) | `failed` (newer attempt) | *(from base row)* | `current` | **composition: the ONE uniquely selected base row's copy** + `· last intake attempt failed` | 0 |
| 6 | valid, ≥30d | `ready` (same) | same | `due` | `Last … N days ago — monthly refresh due` | +1 |
| 7 | valid, ≥30d | `failed` (newer) | *(from base row)* | `due` | **composition: base row's copy** + `· last intake attempt failed` | +1 |
...
| 13a | valid, ≥30d | `review_required` (same) | **none** | `due` | `Last … N days ago — monthly refresh due · latest drop awaiting data review` | +1 |
| 13b | valid, ≥30d | `review_required` (same) | **older offering** | `due` | `Last … N days ago — monthly refresh due · latest drop awaiting data review · analysis uses the <date> drop` | +1 |
...
| 15 | valid older clock (current **or** due by its own age) | newest attempt **invalid** | ≤ clock | clock's own state | **composition: the ONE uniquely selected base row's copy** + `· latest drop's refresh time unverifiable` | per clock state |
...
**PLEASE REPLY with: (a) CLEAR on Phase A framing v8 with checks run, OR (b) numbered findings.**
