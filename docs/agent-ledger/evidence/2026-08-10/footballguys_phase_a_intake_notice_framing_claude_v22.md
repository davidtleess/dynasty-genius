# Phase A framing v22 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v21 (`ae0cabcf…`). Responsive to the
Codex round-21 review: three findings, **ACCEPTED 3/3, zero contested** — a round where Codex ran
LIVE SQLite probes and falsified my `mode=ro` = non-creating claim on the real runtime (reading
committed WAL state CREATES the absent `-shm`; `immutable=1` avoids it but silently loses
committed WAL rows). **v22 distinguishes logical-row read-only from filesystem side-effect-free,
makes quarantined evidence a first-class reducer outcome, and completes the state table with the
conflict and integrity-failure rows.** **Framing only — no RED,
...
- **Existing** — opened `mode=ro` and validated (governed schema, `wal` journal); its committed
  rows count, **and the PERMITTED physical residue is enumerated: creation/mutation of `<db>-shm`
  (the WAL read index) and WAL-recovery bookkeeping, NOTHING else — no main-file page change, no
  row change, no `-wal` growth**;
...
BARRED from clock/AR/copy fallback** until an explicit governed repair/adjudication removes the
failure — dropping invalid evidence and letting a weaker surviving record render `current` with
...
| 19 | **integrity failure** — an identity-invalid or object-invalid persisted row (round-21 R21-2) | *(the invalid row + barred siblings)* | held at last unambiguous value | `unverifiable` | `Footballguys drop record failed integrity check · last unambiguous refresh recorded N days ago` *(clauses drop when no prior clock/AR exists; older-AR variant appends the dated analysis clause)* | +1 |
...
**Conflict/integrity row semantics (round-21 R21-3):** rows 18a-c and 19 are stage-1 base rows —
...
every 18/19 axis in both query orders · the alias-to-row-9 mutant FAILS · the
healthy-clock-fallback mutant FAILS.
