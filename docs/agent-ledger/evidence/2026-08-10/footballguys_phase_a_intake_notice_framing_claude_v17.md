# Phase A framing v17 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v16 (`a43711d2…`). Responsive to the
Codex round-16 review: four findings, **ACCEPTED 4/4, zero contested** — all four are the new
option-3 mechanism not yet INTEGRATED: it lived as prose beside a numbered lifecycle that still
unconditionally published and committed receipts. **v17 makes the mode a BRANCH, not a caveat**,
scopes the crash matrices per branch, makes the unlink crash-DURABLE (directory fsync before the
first provider byte), gives the copy an explicit referent, and closes the WAL-establishment
boundary. **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (R10-3; contract closed per round-11 R11-2/R11-4),
   ...
   **— TERMINAL BRANCH B (retention option 3; round-16 R16-1 — replaces steps 3–6 entirely: no
   object path, no receipt transaction) —**

   **B1.** Create the staging entry exclusively (`O_CREAT|O_EXCL|O_NOFOLLOW`), **`unlinkat` it,
   and DURABLY FSYNC the bound staging-directory descriptor — all BEFORE the first provider byte
   streams in (round-16 R16-2):** my v16 claim of "every crash state" used ordinary process-death
   semantics while this same section admits pre-fsync directory-entry uncertainty; with the fsync
   ordered before the first byte, a SYSTEM crash at any later point recovers no named
   provider-bearing inode, and a crash inside the create→unlink→fsync window strands at most an
   empty named file.
   **B2.** Stream, bound, and hash on the held **anonymous** descriptor; run §4 validation and the
   §2-derived facts exactly as in the common prefix.
   **B3.** **CLOSE the anonymous descriptor** — the kernel reclaims the inode — **before** any
   state becomes visible.
   **B4.** Commit the **observation transaction** to `observations.db` LAST. Only that commit
   advances freshness or renders copy.
   **Option-3 crash matrix (separate; the A-branch matrix and its one-object/one-receipt
   convergence invariant are SCOPED TO OPTIONS 1/2):** crash in B1's window → at most an empty
   named file, swept as an orphan · crash during B2/B3 → the anonymous inode is reclaimed; nothing
   named, nothing durable, no observation · observation-commit failure → no state advance, nothing
   on disk. **Mutants (each must FAIL):** a publish under option 3 · a receipt transaction under
   option 3 · a linked provider-byte-bearing staging entry at any point after B1 · a still-open
   raw descriptor at observation commit · the directory fsync omitted or moved after the first
   byte (asserted through an injected filesystem oracle — a SIGKILL-only probe is explanatory,
   since it passes the broken durability ordering).
