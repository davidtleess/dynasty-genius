# Phase A framing v12 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v11 (`56a807dd…`). Responsive to the
Codex round-11 review: five findings, **ACCEPTED 5/5, zero contested** — a round in which Codex
REPRODUCED its counterexamples with live filesystem probes (check-then-unlink deleting a sentinel;
two processes holding "the" exclusive flock on distinct inodes; a resolving sweep deleting a
symlink's target). **The Critical's honest closure is a THREAT-MODEL NARROWING, not a mechanism:**
POSIX offers no unlink-by-descriptor, so no prose can make a pathname "bound" — the contract now
says exactly what the kernel can enforce and what the private, lock-serialized namespace assumes. **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (R10-3; contract closed per round-11 R11-2/R11-4),
   ...
   re-verify plus never-unlink close that within the declared threat model. Kernel lock release on
   process death is the staleness rule — **never PID reuse or age heuristics**.
   **Contention is DETERMINISTIC (R11-4): `LOCK_NB` failure ⇒ an immediate named `intake_busy` —
   ...
   row by design — that absence is now a stated invariant, not a gap. No blocking mode exists in
   v1; the caller retries when it chooses. RED: hold lock A, replace/alias the lock pathname,
   start B — B must not enter the sweep or staging lifecycle; plus two overlapping intakes + one
   crash — the live run is never destroyed and the survivor converges to one-object/one-receipt.
...
4. **If the canonical path already exists — the OBJECT-INTEGRITY BOUNDARY (round-4 R4-4), with the
   ...
   - verified match → **reuse** (the legitimate dedup branch): byte equality is verified through
     ...
     **a non-cooperating process mutating the private namespace is OUTSIDE the contract** —
     detected after the fact by verification failures, never claimed prevented. The round-10
...
**THE SWEEP CONTRACT (round-11 R11-3) — "safely removed" as an executable predicate:** the
staging root is the fixed private `0700` directory; staged names follow a frozen grammar
(`stage-<random>.tmp`); the sweep **enumerates non-recursively through a bound directory
descriptor**, inspects every entry **no-follow** (`fstatat(…, AT_SYMLINK_NOFOLLOW)`), and **never
resolves, never recurses, never opens a target**. Per type: grammar-matching single-link regular
file → reported as a staging orphan, removed via `unlinkat(dirfd, name)` · **symlink → the LINK
ITSELF is `unlinkat`ed; its target is never opened, parsed, mutated, or deleted** · multi-link
regular file → reported and `unlinkat`ed (removes only this name) · directory or special file →
**REFUSED and reported — never recursed, never deleted** · grammar-nonmatching names → reported,
untouched. RED: a symlink to a sentinel (sentinel byte-identical after), a multi-link regular
file, and a directory/special entry.
...
- **Equal-instant rule (round-8 R8-4, equivalence corrected per round-9 R9-1):** distinct valid
  ...
  **The conflict is a READINESS/CONTENT conflict over a CERTAIN instant (round-10 R10-1 — Codex
  ...
  tie-breaking).
- **Observation equivalence key (round-11 R11-5):** a `refresh_observation` carries no role
  records, so its analytical identity for this rule is its **observation archive identity —
  (archive sha256, bytes) plus declared provenance**. Two same-second observations collapse ONLY
  when that identity is equal; **differing OR MISSING identity ⇒ `same_instant_conflict`** —
  absence of proof is never equality. Mutants: same-second observation pairs with equal and with
  unequal archive hashes, both append orders, asserting freshness, readiness, copy, and AR
  independently.
