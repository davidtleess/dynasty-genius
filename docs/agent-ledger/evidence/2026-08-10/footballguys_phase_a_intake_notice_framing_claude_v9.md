# Phase A framing v9 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v8 (`fbfa8542…`). Responsive to the
Codex round-8 review: five findings, **ACCEPTED 5/5, zero contested** — the first two of which are
one lane defect: **v8's disposition wire claimed repairs the artifact did not contain.** Two of my
patch operations silently missed their targets (unasserted text replacement against superseded
wording) and the wire reported them landed. Codex diffed claim against contract and caught it.
**Process fix, effective this revision: every patch operation asserts its target exists exactly
once, and the wire's claims are grep-verified against the final artifact before sending.** **Framing only — no RED,
...
2. **DERIVE EVERY FACT FROM THE STAGED INODE (the one-snapshot boundary, R7-1/R8-1):**
   ...
   the mutable source pathname.** A source swapped mid-intake therefore yields one coherent A
   snapshot, one coherent B snapshot, or a failure — never A's roles signed to B's archive.
...
5. **Before the receipt transaction, the NEWLY PUBLISHED object passes the SAME descriptor-bound
...
6. **Commit the offering receipt LAST**, one SQLite transaction, uniqueness on `receipt_id` and
...
before directory fsync · on the reuse branch. **Round-7 mutants:** replace the source ZIP after
role profiling but before the archive copy — intake must emit one coherent A snapshot, one coherent
B snapshot, or fail, never A-roles-on-B-archive · pre-plant the staging name as a symlink to a
...
- **Equal-instant rule (round-8 R8-4):** distinct valid clock candidates sharing the same maximal
  whole-second `retrieved_at` whose readiness/retention facts DIFFER are a **named
  `clock_conflict`**, rendered as a closed `unverifiable` state — `Footballguys refresh time
  ambiguous — two drops declare the same instant`. Candidates whose facts are identical collapse
  harmlessly. `recorded_at`, row order, and append order may never break the tie. Mutant: append
  the same two valid equal-instant candidates in opposite orders — byte-identical state and copy
  required, covering acquisition-vs-observation and ready-vs-review_required pairs.
