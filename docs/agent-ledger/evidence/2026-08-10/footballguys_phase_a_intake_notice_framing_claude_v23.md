# Phase A framing v23 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v22 (`66fee9fc…`). Responsive to the
Codex round-22 review: three findings, **ACCEPTED 3/3, zero contested.** Codex's byte-level probe
(main 4,096 B `40cf07c5…` and WAL 12,392 B `adf08d98…` byte-identical before/during/after the read;
only `-shm` appeared) handed the repair its own oracle: **v23 freezes the WAL check at the
physical edge, splits row 19 into literal disjoint rows, and — the honest v1 answer — declares
that NO integrity-failure clearing mechanism exists** until a future, separately framed repair. **Framing only — no RED,
...
BARRED from clock/AR/copy fallback. v1 HAS NO INTEGRITY-FAILURE CLEARING MECHANISM (round-22
R22-3): the state is load-bearing until a FUTURE, SEPARATELY FRAMED repair contract exists** — an
escape phrase without a mechanism is an invitation, and the only detailed adjudication contract in
this artifact (semantic assertions) does not govern acquisition rows. Source rows remain
immutable. REDs: direct row deletion or edit · an unauthenticated `repaired=true` flag · a
replayed row with a changed outcome — NONE may clear the integrity state. *(Previously this
sentence said "until an explicit governed repair/adjudication removes the failure" — deleted as
the unrestricted escape it was.)* Dropping invalid evidence and letting a weaker surviving record
render `current` with
