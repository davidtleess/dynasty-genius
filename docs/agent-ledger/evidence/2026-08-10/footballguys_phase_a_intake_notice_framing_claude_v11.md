# Phase A framing v11 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v10 (`54f41416…`). Responsive to the
Codex round-10 review: five findings, **ACCEPTED 5/5, zero contested** — including R10-1, where
**Codex corrected its own round-9 remedy** and this lane's adoption of it: an equal-instant tie
makes the TIME certain and the CONTENT conflicted, so `clock_conflict`-as-unverifiable was the
wrong model on both sides of the review. Both lanes missed it for one round; the falsification
discipline caught it on the next. **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (round-10 R10-3)** — an exclusive `flock` on a
   per-source lockfile, acquired BEFORE any sweep and held through staging, publication or reuse
   cleanup, receipt commit, and terminal cleanup. Kernel lock release on process death is the
   staleness rule — **never PID reuse or age heuristics**. A second intake finding the lock held
   waits or returns a named `intake_busy`; it NEVER sweeps. Control: two overlapping intakes plus
   one crash — the live run is never destroyed, and the surviving/next run converges to the
   one-object/one-receipt contract.
...
4. **If the canonical path already exists — the OBJECT-INTEGRITY BOUNDARY (round-4 R4-4), with the
   ...
   - verified match → **reuse** (the legitimate dedup branch): byte equality is verified through
     ...
     staging DIRECTORY ENTRY is verified no-follow against the descriptor's device/inode, and THAT
     BOUND ENTRY is unlinked under the lifecycle lock, parent fsynced, before the descriptor
     closes and the receipt commits (round-10 R10-4).** If the entry no longer names the verified
     inode (a concurrent rename), cleanup REFUSES without deleting the replacement and reports the
     displaced staging state — the validation-to-cleanup pathname race is the read-side race
     already barred, and unlinking an unverified name is the same defect pointed backwards.
     Mutant: replace the staging entry after byte equality but before unlink — the sentinel
     replacement stays byte-identical, no receipt commits, no verified inode is silently leaked.** A literal reading of v9 leaked one full paid ZIP per
