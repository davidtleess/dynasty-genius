# Phase A framing v16 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v15 (`8d3be7a4…`). Responsive to the
Codex round-15 review: four findings, **ACCEPTED 4/4, zero contested** — all four in the
conditional-storage model v15 made explicit, including an honesty defect: under option 3 a crash
could leave a paid ZIP on disk while the surface says "no data retained." **v16 closes that with
MECHANISM, not weaker copy** — under option 3 the staging entry is unlinked BEFORE the first
provider byte streams into it, so no provider byte ever has a linked name. **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (R10-3; contract closed per round-11 R11-2/R11-4),
   ...
   **OPTION-3 STAGING EARNS ITS COPY BY MECHANISM (round-15 R15-2):** under option 3 the staging
   entry is created exclusively and **immediately UNLINKED — before the first provider byte
   streams into it** — with all hashing and validation proceeding on the held anonymous
   descriptor; process death reclaims the inode, and **no canonical publish path exists in this
   mode.** A crash in the create-to-unlink window can strand only an EMPTY file, which retains
   nothing. The `metadata only — no data retained` copy is therefore earned across every crash
   state, not asserted. REDs: crash injection during and after option-3 staging, asserting BOTH
   the directory contents (no provider-byte-bearing entry) AND the rendered copy.
