# Phase A framing v25 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v24 (`b8cd59d7…`). Responsive to the
Codex round-24 review: **ONE High finding, ACCEPTED 1/1** — the derived-state model was accepted
on substance, and the sole blocker was my "changed-outcome replay" RED naming a field the frozen
signature does not contain: "outcome" was an undefined proxy, the passes-broken-code species this
framing exists to exclude. **v25 deletes the phantom oracle** (Codex option 1): the real controls
are the per-signed-field negative vectors and the global offering-conflict rule, and derived
readiness/outcome state is explicitly NOT part of acquisition identity. **Framing only — no RED,
...
## 0. Disposition — round 24

| # | Finding | Disposition |
| :-- | :-- | :-- |
| R24-1 | the "changed-outcome replay" RED requires a conflict the frozen identity cannot express | **ACCEPT — phantom oracle deleted; named signed-field controls stand; derived state ≠ acquisition identity, §6 B4** |

**R24-1, conceded:** "outcome" was a word wearing a field's clothes — no persisted field closes
that oracle, so any test claiming it green would in fact be mutating some signed field and testing
the already-frozen rule. The acquisition signature's acquisition-only boundary is load-bearing
(freshness, later semantic evidence) and is NOT widened; if outcome persistence is ever wanted, it
is a separately framed versioned evaluation record, not a silent signature change.

## 0-prev23. Disposition — round 23

| # | Finding | Disposition |
| :-- | :-- | :-- |
| R23-1 | **Critical** — "no clearing mechanism" requires a durable latch the store model does not define | **ACCEPT — derived-state-only model (Codex option 2), argued from the declared threat model, §6 B4** |

**R23-1, conceded:** the same defect shape as round 12's lock-replacement mutant — my own required
control (direct-delete must not clear) demanded behavior my own storage model could not supply.
The consistent resolution is also the same: the mutant is retired as out-of-model, and the promise
narrows to what the evidence can actually carry. A latch was considered and rejected: it defends
against an actor who could equally delete the latch — machinery that cannot survive its own threat
is weight, not safety.

## 0-prev22. Disposition — round 22

| # | Finding | Disposition |
| :-- | :-- | :-- |
| R22-1 | the WAL RED constrains directory names, not the bytes the prose protects | **ACCEPT — byte-level (size, SHA-256) oracle; "bookkeeping" phrase removed, §6 B4** |
| R22-2 | row 19 packs three reachable states behind optional clauses | **ACCEPT — split 19a/19b/19c, literal copy, §7** |
| R22-3 | "governed repair/adjudication" is an unrestricted escape | **ACCEPT — v1 HAS NO CLEARING MECHANISM; future repair = separate framing, §6 B4** |

**R22-1, conceded:** I promised byte preservation and tested filename membership — the oracle
tested the promise's shadow (my recurring class, at the storage layer). Codex's own probe measured
the stronger check: both files byte-identical throughout, only SHM new. **R22-3, conceded:** an
escape phrase without a mechanism is an invitation — the laundering path reopened through the
repair door. The honest v1 rule is that the door does not exist.

## 0-prev21. Disposition — round 21

| # | Finding | Disposition |
| :-- | :-- | :-- |
| R21-1 | `mode=ro` is not filesystem-non-creating (live probe: reads create `-shm`); orphan-sidecar state unclassified | **ACCEPT — enumerated-residue contract; orphan sidecars = malformed, §6 B4** |
| R21-2 | quarantining an invalid row lets a weaker sibling win the clock | **ACCEPT — integrity failures are reducer citizens; siblings barred, §6 B4** |
| R21-3 | `offering_identity_conflict` is a reachable state with no table row | **ACCEPT — conflict + integrity base rows 18a-c/19, §7** |

**R21-1, conceded:** I equated SQLite's logical read-only mode with physical non-creation; the
kernel-adjacent truth is that WAL reads may materialize the shared-memory index. The contract now
says which physical residue is PERMITTED and proves which rows are untouched, instead of promising
an absence the engine does not offer. **R21-2, conceded:** dropping invalid evidence and letting a
weaker record win is exactly the laundering the reducer exists to prevent — quarantine is a state,
not a deletion.

## 0-prev20. Disposition — round 20

| # | Finding | Disposition |
| :-- | :-- | :-- |
| R20-1 | **Critical** — the counterpart lookup can CREATE the forbidden database; no absent-vs-corrupt distinction | **ACCEPT — non-creating tri-state lookup, §6 B4** |
| R20-2 | receipts receive precedence without their own identity recomputation | **ACCEPT — every row of BOTH types recomputes before precedence, §6 B4** |
| R20-3 | the global offering conflict binds writes, not restored/persisted state | **ACCEPT — union grouping before clock candidacy, §6 B4** |

**R20-1, conceded:** my rule said "the other store is checked" as if checking were free of side
effects — on SQLite an ordinary connect IS a write. **R20-2, conceded:** object verification and
identity verification are different facts, and precedence needs both. **R20-3, conceded:** a
write-side rule assumes the writer; restores and corruption do not sign the contract.

## 0-prev19. Disposition — round 19 *(restored here — lost from v20's header by the aborted edit)*

| # | Finding | Disposition |
| :-- | :-- | :-- |
| R19-1 | **Critical** — acquisition identity unique per store, not across retention modes | **ACCEPT — cross-store rule: global conflict check + read-side coalescing with receipt precedence** |
| R19-2 | the reuse branch committed the receipt before the declared LAST transaction | **ACCEPT — premature commit deleted (surviving-sibling instance twelve); one shared transaction + call trace** |
| R19-3 | the observation-ID REDs could not distinguish the frozen formula from `SHA256(offering_id)` | **ACCEPT — named known-answer, per-field vectors, recompute-on-load** |

## 0-prev18. Disposition — round 18
...
4. **If the canonical path already exists — the OBJECT-INTEGRITY BOUNDARY (round-4 R4-4), with the
   ...
   - verified match → **reuse** (the legitimate dedup branch): byte equality is verified through
     ...
     closed — **then reuse FLOWS TO the one shared step-6 transaction. The "receipt commits" that
     lived here was a premature commit beside the step-6 LAST rule (round-19 R19-2,
     surviving-sibling instance twelve — deleted). Transaction-call trace for BOTH A paths: exactly
     ONE receipt transaction per intake, after every filesystem invariant and cleanup; a second
     idempotent insert FAILS the oracle rather than passing as harmless.** **STATED HONESTLY (round-11 R11-1): the final check and the unlink
...
**THE CROSS-STORE ACQUISITION RULE (round-19 R19-1 — one identity means ONE acquisition,
regardless of which database holds its rows):**
- **write side, global:** before committing EITHER row type, the other store is checked for the
  same `offering_id` under the held lifecycle lock — identical signature in `receipts.db` makes a
  new observation commit a **no-op** (the acquisition already exists at a stronger retention
  state); identical signature in `observations.db` lets the receipt commit (the 3→1/2 upgrade);
  **any differing signed field under the same offering is `offering_identity_conflict` GLOBALLY**,
  not per store;
- **read side:** the reducer **COALESCES cross-store records sharing one acquisition-signature
  identity into ONE effective acquisition — never a `same_instant_conflict`, because one identity
  is one acquisition, not a tie**; a hash-verified receipt supplies the retained/readiness/AR
  state (receipt precedence: retention is PROVEN by the verified object); the coalesced
  acquisition is one clock candidate;
- **copy:** where an identical-signature verified receipt exists, the observation's "its archive
  was not retained" can never render — the coalesced state is the receipt's, and the copy follows
  the effective state.
**THE COUNTERPART LOOKUP — LOGICAL READ-ONLY WITH ENUMERATED PHYSICAL RESIDUE (round-20 R20-1;
corrected per round-21 R21-1 — Codex's live probe: reading committed WAL state through `mode=ro`
CREATES an absent `-shm`, and `immutable=1` avoids that while silently LOSING committed WAL rows —
so `immutable=1` is BANNED unless a control ever proves uncheckpointed committed rows visible):**
the lookup distinguishes **logical-row read-only** (no schema or application row may change —
proven) from **filesystem side-effect-free** (NOT promised for an existing WAL database). The
classification:
- **Absent = main file AND every sidecar absent** — detected by no-follow existence checks through
  the verified dirfd, no SQLite connection of any kind opened — means empty; all three files
  REMAIN ABSENT after the commit;
- **main file absent + ANY sidecar present = MALFORMED/unverifiable, never empty** — restored or
  corrupt residue; the commit fails closed, all state unchanged;
- **Existing** — opened `mode=ro` and validated (governed schema, `wal` journal); its committed
  rows count, **and the PERMITTED physical residue is exactly: creation/mutation of `<db>-shm`,
  NOTHING else** *(the "WAL-recovery bookkeeping" phrase is REMOVED per round-22 R22-1 — on the
  measured runtime all recovery materialized in the SHM while main and WAL stayed byte-identical,
  and the broader phrase could hide exactly what "nothing else" forbids)*. **The oracle is frozen
  at the physical edge: main and `-wal` `(size, SHA-256)` recorded BEFORE open, WHILE open, and
  AFTER close — all three snapshots byte-equal for both files, only `-shm` may appear or change —
  and schema/application rows byte-logically unchanged.** Directory membership is not the test:
  filename deltas cannot see a mutated unused main page or a same-size WAL overwrite. Mutants:
  mutate an unused main page after the read · truncate the WAL · overwrite same-size WAL bytes —
  each FAILS even though the selected row still reads correctly;
- **Unreadable/malformed/wrong-schema/wrong-journal** — never empty; fail closed, all state
  unchanged.
REDs: **the exact main+WAL/no-SHM shape — the committed WAL row IS seen AND the directory delta is
exactly the permitted set** · all-three-absent non-creation controls (both directions) ·
main-absent+orphan-sidecar refuses · corrupt counterpart refuses · **the create-capable
`sqlite3.connect(other_path)` mutant FAILS on physical non-creation** · an `immutable=1`
implementation FAILS the committed-WAL-row visibility control.
**EVERY ROW VALIDATES ITS OWN IDENTITY BEFORE COMPOSITION (round-20 R20-2):** before global
conflict detection or coalescence, the canonical signature is reconstructed from the persisted
signed fields of **every receipt AND every observation row** and compared to the stored id;
mismatch quarantines the row. **Receipt precedence is eligible ONLY after BOTH its
metadata-identity validation AND its descriptor-bound object verification pass** — object
verification proves the bytes; identity verification proves the claims; precedence needs both.
The known-answer and per-signed-field/stored-id mutants apply to receipt loads too.
**QUARANTINE IS A STATE, NOT A DELETION (round-21 R21-2):** any identity-invalid or object-invalid
persisted row enters the effective reduction as a **named integrity failure**: the stream renders
`unverifiable`, the last unambiguous clock and AR HOLD, and **every same-offering sibling is
BARRED from clock/AR/copy fallback WHILE THE FAILING EVIDENCE PERSISTS. The integrity state is
DERIVED-STATE-ONLY (round-23 R23-1, superseding v23's unimplementable no-clearing promise):**
- **no APPLICATION override exists** — no repair API, no `repaired` flag, no adjudication path
  clears it; that door stays closed exactly as v23 closed it;
- the state **lasts as long as current governed evidence fails validation**, recomputed on every
  load — and **a verified governed restore MAY genuinely clear it**, because evidence restored to
  its exact receipt-bound bytes IS valid again (a restore that heals is a feature of the backup
  contract, not a laundering path — the reducer never dropped the row; the row became true);
- **out-of-band same-uid deletion or mutation of rows/objects is OUTSIDE every guarantee**, per
  the standing rounds-11/12 threat model — v23's direct-delete "state remains" mutant is RETIRED
  as demanding memory no in-model store carries (the round-12 lesson recurring: my own control
  required behavior my own model excludes). A durable latch was considered and REJECTED: the actor
  who deletes the row deletes the latch.
In-model REDs that stand: an unauthenticated `repaired=true` flag clears nothing · **replay
identity is tested by the NAMED signed-field controls — the per-signed-field negative vectors
(§6a) and the global offering-conflict rule — each naming its exact mutated field, unchanged
fields, and expected identity/reducer result; the former "changed-outcome replay" RED is DELETED
(round-24 R24-1): "outcome" is not a signed field, DERIVED READINESS/OUTCOME STATE IS NOT PART OF
ACQUISITION IDENTITY, and an oracle over an undefined proxy is the passes-broken-code species** ·
the filter-before-reduce and sibling-wins mutants still FAIL while failing evidence persists ·
**a verified exact-byte restore clears the state AND the pre-restore reduction provably showed
19a-c** · an UNVERIFIED or wrong-byte replacement stays invalid. Dropping invalid evidence and letting a weaker surviving record
render `current` with
"its archive was not retained" is precisely the laundering the reducer exists to prevent (a
receipt whose identity or bytes are untrustworthy still proves an archive was MEANT to be
retained). The cross-store fixture asserts **status, clock identity, pill, FULL copy, and Phase-C
closure** — never only "no coalescence, no AR advance"; mutants that filter the quarantined row
before reduction or let the sibling observation win MUST FAIL.
**THE GLOBAL CONFLICT BINDS PERSISTED STATE (round-20 R20-3):** after per-row recomputation and
before clock candidacy, the UNION of both stores is grouped by `(source, offering_id)`: one
distinct valid signature may coalesce; **more than one is the named global
`offering_identity_conflict` — the stream renders `unverifiable`, AR and the last unambiguous
clock HOLD, and the group never enters equal-instant selection** (clock ranking may never launder
an offering conflict into a normal acquisition or a timing tie). REDs: an already-persisted
conflicting pair in both query orders · a skewed independent-backup restore · **a test proving
only prospective write refusal MUST FAIL.**
Local REDs (unchanged): both transition directions with identical signature (one effective clock
candidate, deterministic AR/copy, no false conflict) and with conflicting signature (global
refusal) · crashes between the two stores · **a test that checks each database separately and
never the composition MUST FAIL.**
**THE IDENTITY FORMULA IS PROVEN, NOT SHADOWED (round-19 R19-3 — `SHA256(offering_id)` passes
every cardinality RED; the formula needs its own oracle):** the §6a positive vector is promoted to
a named observation-ID known-answer — **the positive row's `observation_id` MUST equal
`0d6bf3065b2c432f64262e7de427c064eda121cb1014c20d74d1fce4e3ef596e`**; the §6a signed-field
negative vectors apply to observation identity with NEW offering ids where needed, so
UNIQUE(offering_id) can never mask a bad formula; **on every load the id is recomputed from the
persisted signed fields and a mismatch refuses/quarantines**; mutants: each signed field mutated
independently, and the stored id mutated alone.
Local REDs (unchanged): two sequential identical B intakes → one row, one clock candidate ·
...
| 18a | **global `offering_identity_conflict`** (persisted, >1 valid signature per offering), **no prior unambiguous clock** | *(the conflicting group)* | none | `unverifiable` | `Footballguys drop records conflict — one drop declared with differing identities · no unambiguous refresh recorded` | +1 |
| 18b | offering conflict, **held prior unambiguous clock**, no AR | *(conflicting group)* | none | `unverifiable` | `Footballguys drop records conflict — one drop declared with differing identities · last unambiguous refresh recorded N days ago` | +1 |
| 18c | offering conflict, held prior clock, **older AR** | *(conflicting group)* | **held older receipt** | `unverifiable` | `Footballguys drop records conflict — one drop declared with differing identities · last unambiguous refresh recorded N days ago · analysis uses the <date> drop` | +1 |
| 19a | **integrity failure** (identity- or object-invalid persisted row, R21-2), **no prior unambiguous clock** | *(the invalid row + barred siblings)* | **none** | `unverifiable` | `Footballguys drop record failed integrity check · no unambiguous refresh recorded` | +1 |
| 19b | integrity failure, **held prior unambiguous clock**, no AR | *(invalid row + barred siblings)* | **none** | `unverifiable` | `Footballguys drop record failed integrity check · last unambiguous refresh recorded N days ago` | +1 |
| 19c | integrity failure, held prior clock, **older AR** | *(invalid row + barred siblings)* | **held older receipt** | `unverifiable` | `Footballguys drop record failed integrity check · last unambiguous refresh recorded N days ago · analysis uses the <date> drop` | +1 |
...
**Conflict/integrity row semantics (round-21 R21-3; row 19 split literal per round-22 R22-2 — one
row covering three states behind optional clauses was the first-match/hidden-axis class the table
exists to eliminate):** rows 18a-c and 19a-c are stage-1 base rows —
newer failed/invalid attempts compose over them via stage 2 exactly once; their copy names the
ACTUAL failure (an acquisition-identity dispute, an integrity failure) and **never aliases to row
9's `record unreadable`**, which describes an unreadable ledger, not two valid conflicting
signatures; held prior-clock and dated older-AR facts are disclosed wherever they exist. REDs:
every 18a-c/19a-c axis in both query orders, plus failed/invalid overlays over each · the
alias-to-row-9 mutant FAILS · the healthy-clock-fallback mutant FAILS · **substring-only and
first-match renderer mutants FAIL** (always-drop-the-dated-clause, always-print-last-unambiguous
where none exists, and no-AR-variant-for-everything each violate exactly one literal row).
...
**PLEASE REPLY with: (a) CLEAR on Phase A framing v25 with checks run, OR (b) numbered findings.**
