# Phase A framing v10 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v9 (`301494a8…`). Responsive to the
Codex round-9 review: five findings, **ACCEPTED 5/5, zero contested**. *(The v9 process fix —
asserted patches, grep-verified wire claims — held: round 9 confirmed every round-8 repair live and
reproduced the preimages from the artifact alone.)* **Framing only — no RED,
...
1. **STAGE THE INTACT ARCHIVE FIRST.** Create the staging file **exclusively and no-follow**
   (`O_CREAT|O_EXCL|O_NOFOLLOW`) under an **unpredictable name** on the same filesystem; stream the
   source within §4's caps, hash while copying, fsync. **The descriptor stays open through every
   following step until after the staged/published inode comparison** — it is the identity being
   verified, and closing it early is the round-8 contradiction this rewrite removes.
2. **DERIVE EVERY FACT FROM THE STAGED INODE (the one-snapshot boundary, R7-1/R8-1):**
...
4. **If the canonical path already exists — the OBJECT-INTEGRITY BOUNDARY (round-4 R4-4), with the
   ...
   - verified match → **reuse** (the legitimate dedup branch): byte equality is verified through
     ...
     they are necessarily different inodes; then **the staging descriptor is closed and the
     redundant staging name/inode UNLINKED with its parent fsynced BEFORE the receipt commits
     against the existing object.** A literal reading of v9 leaked one full paid ZIP per
...
6. **Either branch: commit the offering receipt LAST**, one SQLite transaction, uniqueness on `receipt_id` and
...
**THE POST-CRASH MATRIX (round-9 R9-4) — each injection point names its permitted durable residue
and required restart behavior; "no receipt" alone is never a passing oracle:**

| Crash point | May remain on disk | Restart contract |
| :-- | :-- | :-- |
| during staged write | a partial staging file under the unpredictable name | discovered by the staging-directory sweep at next intake start; **reported as a recoverable staging orphan and safely removed** — never parsed, never a committed offering |
...
| receipt-commit failure (either branch) | a verified canonical object with no referencing receipt | **reported as a recoverable canonical orphan** (the already-stated invariant); adopted by the next same-content intake's reuse branch |
...
canonical object and one valid receipt with no manual pathname surgery.** Each row is a RED
...
- **Equal-instant rule (round-8 R8-4, equivalence corrected per round-9 R9-1):** distinct valid
  ...
  **Different `content_vintage_id` values at the maximal instant CONFLICT even when both rows say
  ...
  collapse only when role vintage and every downstream effect are identical. Any non-equivalent
  tie is a **named `clock_conflict`**, rendered as the closed `unverifiable` base row of §7 —
  copy: `Footballguys refresh time ambiguous — multiple drops declare the same instant` (true for
  any cardinality). `recorded_at`, row order, and append order may never break the tie. **Mutants:**
  the same candidates appended in opposite orders must yield byte-identical state AND identical
  effective-clock and `latest_analysis_ready` identity/content — never merely identical rendered
  copy — covering acquisition-vs-observation, ready-vs-review_required, and **two equal-second
  `ready` receipts with different `content_vintage_id`** (named conflict both orders; neither
  content becomes analysis-ready by tie-breaking).
...
| 16 | **`clock_conflict`** — non-equivalent candidates tied at the maximal instant (any cardinality ≥ 2) | *(the tied candidates)* | **held at the last unambiguous value; never advanced by any conflicted candidate** | `unverifiable` | `Footballguys refresh time ambiguous — multiple drops declare the same instant` | +1 |
...
**`clock_conflict` precedence (round-9 R9-2) — a stage-1 base row like any other:** while the
conflict stands, **AR holds at the last unambiguous value** and no conflicted candidate can advance
it; **an older unambiguous clock never silently wins** — the conflict governs the display (falling
back would let tie-breaking pick football data); **newer failed/invalid attempts compose over the
conflict base row** via stage 2 exactly as over any base; **a strictly LATER unique valid candidate
clears the conflict** by becoming the new maximal clock (the conflicted instant stays recorded
history); a conflict never disappears because query order changed. Controls: two and three tied
candidates · an older prior clock present · a newer failed attempt over the conflict · a later
unique valid acquisition clearing it.
