# Phase A framing v24 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v23 (`5ada58a6…`). Responsive to the
Codex round-23 review: **ONE Critical finding, ACCEPTED 1/1** — v23's "no clearing mechanism"
promise was UNIMPLEMENTABLE: the integrity state is a predicate DERIVED from current evidence, so
deleting the row leaves nothing to derive the failure from, and my required direct-delete mutant
demanded memory no store possessed. **v24 chooses the DERIVED-STATE-ONLY model, argued from the
contract's own threat model** — out-of-band same-uid mutation has been outside every guarantee
since rounds 11-12, and a durable latch could be deleted by the same out-of-band actor it exists
to survive. **Framing only — no RED,
...
In-model REDs that stand: an unauthenticated `repaired=true` flag clears nothing · a replayed row
with a changed outcome refuses (`offering_identity_conflict`) · the filter-before-reduce and
sibling-wins mutants still FAIL while failing evidence persists · **a verified exact-byte restore
clears the state AND the pre-restore reduction provably showed 19a-c** · an UNVERIFIED or
wrong-byte replacement stays invalid. Dropping invalid evidence and letting a weaker surviving record
render `current` with
