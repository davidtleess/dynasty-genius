# Phase A framing v18 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v17 (`bd468682…`). Responsive to the
Codex round-17 review: three findings, **ACCEPTED 3/3, zero contested.** The Critical: v17's
branch split was headings, not control flow — the linked staging step stayed "common" while Branch
B sat textually BEFORE the prefix it claimed to follow. **v18 restructures §6 as an executable
order: common step 0 → step 1 split A1/B1 (staging acquisition IS mode behavior) → one shared
descriptor-bound validation routine → the two terminal branches, in reading order.** Codex's
trace: "read the numbered flow top-to-bottom" — a contract whose headings disagree with its line
order is two contracts. **Framing only — no RED,
...
1. **ACQUIRE STAGING IN THE MODE BRANCH (round-17 R17-1 — staging acquisition IS mode
   ...
   **Call-trace oracle (R17-1): exactly ONE staging create and ONE source stream per intake**, in
   the active mode's shape — the mutant that runs A1 before B1 (two creates, one linked) FAILS.
...
**B2.** The shared routine's refusals apply; **descriptor cleanup is an UNCONDITIONAL
finally-class invariant (round-17 R17-2): EVERY exit after creation — B1/fsync refusal, malformed
archive, resource cap, missing role, CRC/hash failure, schema failure, source read error, success
— closes the anonymous descriptor.** A long-lived process may never retain the paid inode after a
rejected intake. REDs per failure family, keeping the process ALIVE, asserting the raw
descriptor/inode is gone, no observation committed, clock/AR/copy unchanged; the mutant that
removes only the failure-path cleanup FAILS (the success-path-only mutant was the previous gap).
**B3.** On success: **CLOSE the anonymous descriptor before anything becomes visible.**
...
Logical safety property: **no named/raw provider archive survives · no observation row commits ·
clock/AR/copy do not advance.** REDs: inject a REAL SQLite transaction failure, reopen, assert the
