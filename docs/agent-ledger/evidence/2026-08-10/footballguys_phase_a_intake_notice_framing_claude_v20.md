...
- **copy:** where an identical-signature verified receipt exists, the observation's "its archive
  ...
  the effective state.
REDs: both transition directions with identical signature (one effective clock candidate,
deterministic AR/copy, no false conflict) and with conflicting signature (global refusal) ·
crashes between the two stores · **a test that checks each database separately and never the
