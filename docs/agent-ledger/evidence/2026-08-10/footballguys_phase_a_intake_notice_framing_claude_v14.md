# Phase A framing v14 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v13 (`d3f5686a…`). Responsive to the
Codex round-13 review: four findings, **ACCEPTED 4/4, zero contested.** Two are the stale-sibling
class AGAIN — corrected sentences added while the sentences they replaced stayed live — so this
revision adds what my presence-only verification lacked: a **FORBIDDEN-PHRASE sweep** over the
operational sections, verified before send (the retired formulations must occur ZERO times outside
quoted disposition history). **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (R10-3; contract closed per round-11 R11-2/R11-4),
   ...
   **Lock lifecycle, stated truly (round-12 R12-4; boundary made enforceable per round-13
   ...
   it, passing it over any socket, or leaking it to a child is forbidden**; and the production
   lock holder carries an **executable guard** — the lock object's scope asserts that no
   fork-capable call (`fork`, `posix_spawn`, `subprocess`) is made while the lock is held, failing
   loudly if one is attempted. **The RED mutates/removes THAT guard and must fail** — the
   parent/fork/child probe demonstrated only an OS fact any implementation passes, and it is
   demoted to an explanatory OS-semantics control (Codex round-13: a seed that passes broken code
   is the named defect species).
   **The namespace bootstrap and load predicate (round-12 R12-2; identity + creation closed per
   ...
   opened that way**; a symlinked ANCESTOR refuses, not just a symlinked leaf. Load predicate
   unchanged: descriptor-bound type/owner/mode verification, fail closed before any lock, stage,
   sweep, or cleanup, all through the verified dirfd. **Manifest treatment stated:** staging
   content is transient (not a durable store); the lockfile is trivial state; neither enters the
   backup manifest — the receipts/observation ledgers keep their own §6/§8 coverage. RED: missing
   root created correctly with the exact chain semantics · symlinked ancestor refuses · symlinked
   leaf refuses · group/world-writable and wrong-owner roots refuse — all before any lifecycle
   operation.
...
**PLEASE REPLY with: (a) CLEAR on Phase A framing v14 with checks run, OR (b) numbered findings.**
