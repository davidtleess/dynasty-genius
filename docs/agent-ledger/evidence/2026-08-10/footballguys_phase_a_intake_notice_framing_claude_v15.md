# Phase A framing v15 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v14 (`60a681da…`). Responsive to the
Codex round-14 review: five findings, **ACCEPTED 5/5, zero contested** — every one exposed BECAUSE
v14 pinned a concrete repository path, which is what convergence looks like: reality creates
falsifiable surface. The sharpest: Codex ran `git check-ignore` against the pinned namespace and
proved a crash-resident PAID ZIP would be **commit-eligible** — mode bits are no boundary against
Git running as the same uid, and this repo's own `.gitignore` policy demands a deliberate
per-source rule. **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (R10-3; contract closed per round-11 R11-2/R11-4),
   ...
   **EVERY RUNTIME LOCATION IS PINNED (round-14 R14-2):** retained-object root =
   `app/data/footballguys/objects/`, canonical pathname = `objects/<archive_sha256>.zip` (the full
   64-hex archive hash, the frozen hash-to-path grammar); receipt ledger =
   `app/data/footballguys/receipts.db` with its `-wal`/`-shm` companions; semantic
   assertion/evidence store = `app/data/footballguys/semantics.db` (+ sidecars); observation
   ledger (option 3) = `app/data/footballguys/observations.db` (+ sidecars). **Under option 3,
   `objects/` and `receipts.db` DO NOT EXIST.** `staging/` and `objects/` live under one
   filesystem, **asserted (`st_dev` equality through the held descriptors) BEFORE the no-replace
   publish**; cross-device is a refusal and a mutant.
