# Phase A framing v7 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v6 (`c8ca6840…`). Responsive to the
Codex round-6 review: six findings, **ACCEPTED 6/6, zero contested**. **Framing only — no RED,
...
- **offering signature** = the canonical lines `source`, `offering_id`, `content_vintage_id`,
  `retrieved_at` (canonical UTC `YYYY-MM-DDTHH:MM:SSZ`, seconds precision — offset spellings
  normalize BEFORE serialization), `archive_sha256`, `archive_bytes` (decimal, no padding), then
  the two role-record lines; **`receipt_id`** = SHA-256 over those bytes.
...
**Negative vectors (each MUST differ from the positive — the difference is the test):** role swap →
`content_vintage_id` `fb6b16f6…` · `retrieved_at` serialized in offset form instead of normalizing →
`receipt_id` `d5785e03…` (proves normalization is load-bearing) · zero-padded integer →
`receipt_id` `d87163c3…`. Further mutants: object bytes compared to `receipt_id` · only decompressed
roles retained where the contract requires the intact archive · delimiter-collision probe (a value
containing `\n` must be UNREPRESENTABLE, not escaped). **Round-4 R4-4 mutants:** pre-existing hard-link
