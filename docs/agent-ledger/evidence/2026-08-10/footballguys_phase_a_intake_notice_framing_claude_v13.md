# Phase A framing v13 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v12 (`2384dd9b…`). Responsive to the
Codex round-12 review: five findings, **ACCEPTED 5/5, zero contested.** The Critical is the
round-11 narrowing applied CONSISTENTLY: v12 declared namespace mutation out of model and then
required a RED proving resistance to it — a test my own model excludes and my own mechanism cannot
pass (Codex probed it: a stable replacement satisfies both processes' post-checks). **You cannot
keep both the mutant and the model.** The mutant is retired; the model now also says plainly that
out-of-model behavior is outside ALL guarantees and only *potentially* detectable — "detected
after the fact" was overclaimed and is corrected. **Framing only — no RED,
...
0. **ACQUIRE THE PER-SOURCE LIFECYCLE LOCK (R10-3; contract closed per round-11 R11-2/R11-4),
   ...
   **The lock RED is IN-MODEL only (round-12 R12-1):** the replacement/alias control is RETIRED —
   ...
   intakes + one crash — the live run is never destroyed and the survivor converges to
   one-object/one-receipt. **Out-of-model namespace mutation is outside ALL guarantees and only
   POTENTIALLY detectable later** — v12's "detected after the fact" was overclaimed and is
   corrected here.
   **Lock lifecycle, stated truly (round-12 R12-4):** `flock` releases when the LAST reference to
   the locked open file description closes — process death releases it only if no descendant
   inherited the descriptor. **The intake process MUST NOT fork while holding the lifecycle
   lock**; that process-model rule is frozen and tested (RED: parent acquires, forks an inheriting
   child, parent exits — the conforming implementation never does this, and the control proves the
   hazard the rule exists to prevent: acquisition stays blocked until the child exits, so a
   violation strands the source in `intake_busy`).
   **The namespace bootstrap and load predicate (round-12 R12-2)** — the private root the whole
   threat model stands on is ESTABLISHED and VERIFIED, never assumed: fixed path; opened
   `O_DIRECTORY|O_NOFOLLOW`; descriptor-bound `fstat` requires directory type, owner == the intake
   uid, and mode exactly `0700`; **refusal on a symlinked root, ownership mismatch, or any broader
   permission** — fail closed before any lock, stage, sweep, or cleanup operation, all of which
   run through this verified dirfd. RED: a symlinked root, a group/world-writable root, and a
   wrong-owner root each refuse before any lifecycle operation.
