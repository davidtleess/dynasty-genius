# Phase A framing v21 — Footballguys archive intake + monthly refresh notice (Claude)

Date: 2026-08-10 · **Layer 1 (ingest).** Supersedes framing v20 (`6ad2af3d…`) — **whose title and
header still read "v19": the v20 edit script aborted mid-run, its successful header replacements
were lost with it, and my claim-verification probed only the new content, not the header. The
content repairs all landed; the header did not. Disclosed here rather than smoothed** — the same
transactional-loss class as round 11's, and the header is now a standing verification probe.
Responsive to the Codex round-20 review: three findings, **ACCEPTED 3/3, zero contested.** The
Critical is a classic SQLite trap my cross-store rule walked into: `sqlite3.connect` CREATES an
absent database, so the mandatory counterpart lookup could create the very store the active
retention mode forbids. **Framing only — no RED,
...
- **copy:** where an identical-signature verified receipt exists, the observation's "its archive
  ...
  the effective state.
**THE NON-CREATING COUNTERPART LOOKUP (round-20 R20-1 — on SQLite an ordinary connect IS a
write):** the lookup is tri-state. **Absent** — detected by a no-follow existence check through
the verified namespace dirfd, with NO write-capable connection ever opened — means empty, and the
absent database with its `-wal`/`-shm` REMAINS ABSENT after the commit. **Existing** — opened
READ-ONLY (`mode=ro` URI) and validated (governed schema, `wal` journal mode) before its rows
count. **Unreadable/malformed/wrong-schema/wrong-journal** — NEVER treated as empty: the commit
fails closed with both stores, clock, AR, pill, and copy unchanged. REDs: option 3 with absent
`receipts.db` commits one observation while `receipts.db`/`-wal`/`-shm` remain absent · the
symmetric option-1/2 direction · corrupt/unreadable/wrong-schema counterpart refuses · **the
mutant using an ordinary create-capable `sqlite3.connect(other_path)` FAILS on physical
non-creation.**
...
The known-answer and per-signed-field/stored-id mutants apply to receipt loads too; cross-store
fixture: a valid observation beside a receipt with a hash-valid object but a mismatched recomputed
id — no coalescence, no AR advance.
