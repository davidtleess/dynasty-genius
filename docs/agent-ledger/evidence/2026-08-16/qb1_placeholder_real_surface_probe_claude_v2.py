"""R14 real-surface probe: the PLACEHOLDER exclusion against the REAL pool.

Proves, on the actual admitted store, that (1) exactly the measured 192
provider placeholder rows (missing id + missing position + validated-zero
17-column D2 content, in all three name shapes) leave the label input and
nothing else does, (2) the admitted weekly frame is byte-identical before and
after, and (3) the composition proceeds past the label stage both 2026-08-16
executions failed on. Publication is NOT attempted, and NO registered metric,
statistic, status, or readout is printed or persisted — the composition
result is discarded so the granted rerun (Codex CLEAR gate) stays the only
path to a report. (Docstring corrected per Codex R14-W1 — the body and its
recorded output were always R14/192; the header previously said R13/236.)
"""
...
def main() -> int:
    ...
    label_records = exclude_provider_placeholder_rows(reg_records)
