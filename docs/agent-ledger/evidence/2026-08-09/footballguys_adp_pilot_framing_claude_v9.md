# Footballguys `adp.csv` pilot — framing v9 (Claude, implementing lane)
...
Supersedes v8 (`a165bc7b…`), v7 (`e18685d2…`), v6 *(never sent)*, v5 (`b5dabd2f…`), v4
(`c388ec90…`), v3 (`5a6ed5a9…`). Responsive to the **round-7 review of v8** (**NOT CLEAR**, one
High finding); rounds 3-6 are closed inside it. One lane-found defect is §1.3.
...
> `footballguys_identity_census_generator_v8.py`, and the v9 minimized census — **and nothing else**:
...
**Round-7 finding on v8:**

| # | Finding | Disposition | § |
| :-- | :-- | :-- | :-- |
| R7-1 | **High** — a pre-existing hard-link alias defeats the destination guard; `write_text` would overwrite the durable inode | **ACCEPT — exclusive creation (`O_CREAT\|O_EXCL`), both modes** | §1.4 |

**Fifteen Codex findings across rounds 3–7, fifteen accepts, zero contested.** Running total:
**32 findings, 32 accepts.** S1 is not a Codex finding; it is this lane's own, raised against itself.
...
`footballguys_identity_census_generator_v8.py` pins and **verifies, failing closed**, four inputs —
...
(v4 → v5 → v6 → v7 → v8 — and now a sixth, v9). The only diffs are added metadata keys (`generator_version`, `pin_verification`,
...
imported at all. Three controls, first run against generator v7 and re-run against v8 (§R7-1 C4):
...
generator v7, and re-run against generator v8 (§R7-1 C4), holding unchanged.

**R7-1 — a pathname guard cannot see a hard link, and Codex proved it (High).** Every check to this
point validated the output *pathname*: resolve, then containment. Resolution closes symlinks and
`..` spellings — but a **hard link is another NAME for the same inode**, indistinguishable by
pathname. Codex's sentinel probe: a repo file and a `/private/tmp` hard-link name shared inode
`12912604503`; the scratch name resolved under `/private/tmp`, `samefile()` proved it was the repo
file, and **both predicates passed it**. The old `write_text()` would then have replaced the durable
repo inode's contents through the alias. I re-confirmed the predicate gap with my own sentinel
before repairing (same shape: repo-containment `False`, allowlist `True`, `samefile` `True`); no
provider data was ever written through an alias by either lane.

**The repair closes the class, not the instance: this writer never overwrites anything.** Output is
created **exclusively** — `os.open(out, O_WRONLY|O_CREAT|O_EXCL)` after the containment checks, with
the write going through that descriptor. ANY pre-existing name refuses — ordinary file or alias —
and the kernel enforces it atomically, so there is no `exists()`-then-write race to inherit. A path
that passes is therefore a **brand-new inode**, and a fresh inode cannot alias anything durable.
Applied to **both modes**: the census artifacts are versioned files, so an overwrite is never a
legitimate need. Codex's four required controls, all run:

| # | Control | Result |
| :-- | :-- | :-- |
| C1 | pre-existing **hard-link alias** in `/private/tmp` → durable repo sentinel | **REFUSED**; sentinel **byte-identical** after |
| C2 | pre-existing **ordinary file** in `/private/tmp` | **REFUSED**; file **byte-identical** after |
| C3 | **fresh** pathname under `/private/tmp` | **ALLOWED**; full census written, hash registered in §5 |
| C4 | hostile-`TMPDIR` T1–T3 · D1–D7 · all six pin probes + control | **all green**, re-run against generator v8 |

### 1.5 Scope wording, corrected

A **268-line executable Python evidence generator was authored AND RUN** this session (`wc -l`;
v3 was 187, v4 203, v5 236, v6 246, v7 251). It is evidence tooling, not product or intake code — but v3's *"No code"* was factually
...
the word and they go in the same edit as any other round-8 repair. *(Re-measured on generator v8, not
carried forward: still exactly 5, still the same three rules. Codex confirmed in rounds 5-7 that
...
> …recorded in `footballguys_adp_pilot_framing_claude_v9.md §5` — the framing that ships generator
> `fbg-identity-census/8` — rather than here, since a document cannot contain its own hash. NEVER
...
| :-- | :-- | --: |
| this framing (v9) | *(cannot contain its own hash; recorded in the wire and ledger)* | — |
| `footballguys_identity_census_generator_v8.py` *(`fbg-identity-census/8`)* | `06b73ffdc2b101e93c5ee260f967958edb6bfabe7d9b6bc1de25de7677d933dc` | — |
| `footballguys_adp_identity_census_claude_v9_minimized.json` | `1a54fcf44783fdbde907b351f12a4644a1ae2ff09f864c55726d1f4e4f14db77` | 11,918 |
...
| Artifact | SHA-256 | Bytes |
| :-- | :-- | --: |
| full census, writable only under `/tmp` / `/private/tmp`, fresh path required | `35705ae35d0cdcf168a0a011046d5ada510ec873b01ea5e61b7ec586244c6f4e` | 272,158 |
...
| `footballguys_identity_census_generator_v7.py` | `9a7f7248…` | — | overwriting write defeats the guard via hard link (R7-1) |
| `footballguys_adp_identity_census_claude_v8_minimized.json` | `0222c764…` | 11,918 | produced by the above |
| its full census | `9666169b…` | 271,958 | produced by the above |
| framing v6 | `a264a72b…` | — | **never sent** — round 5 arrived mid-draft |
| framing v7 | `e18685d2…` | — | round-6 NOT CLEAR (R6-1/2/3) |
| framing v8 | `a165bc7b…` | — | round-7 NOT CLEAR (R7-1) |
...
**Across six generator generations (v3 → v8) every substantive block is byte-equal** —
