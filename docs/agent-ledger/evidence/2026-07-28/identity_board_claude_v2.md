# TW28-IDENTITY-1 — Identity: what exists, what it costs today, what to do first (v2)
...
**v2 supersedes v1** (`identity_board_claude_v1.md`, SHA-256 `9c873fded263…`) after Codex's
NOT-CLEAR challenge (`identity_board_codex_challenge_v1.md`) and my written disposition
(`identity_board_claude_disposition_v1.md`). All eight required corrections are folded in. **Two of
Codex's findings are better than anything in v1**, and one of them made v1 wrong in a worse
direction than it was written — both are called out below rather than quietly absorbed.
...
**I-4 · Protect the exact vintage.** Backup-manifest entry, or re-pin as a committed hash-stamped
snapshot. **Honest ambiguity, not a settled exemption (Codex ch.12):** the file is re-pullable from
nflverse, which *may* put it outside the manifest law's narrow disaster-loss mandate — but the source
is mutable GitHub `master` and the snapshot records only a pull timestamp, **no upstream commit
SHA**, so the exact operational input behind every shipped value cannot be re-pinned even in
principle. Both halves are true; the call is David's. *Gate: David.*
