# PlayerProfiler `player_season` observed-change pilot (N6) — protocol v5

**Author:** Claude Code, implementing lane · **Layer:** Layer 1 (ingest) inventory
**Supersedes:** v4 (`1da37806…`, NOT CLEAR on U1–U2) · v3 · v2 · v1
...
Codex directed must remain · the expanded preparation+digest prerequisite (T1) · **§4.6's truncation
disposition, which the reviewer confirmed is correct: disclose the unmitigated threat, do not invent
a row-count classifier.**
...
**⚠ AND ONE PROPERTY THAT MAKES THIS SHARPER THAN T1 STATES — narrowed at U2 to what it actually
supports.** The `seen` dedup is **first-occurrence-wins and therefore order-dependent**, and that
ordering must be **preserved across `player_season` exports and rows**: a pure helper that reorders
them will silently keep a **different surviving row** than production did, and digests will diverge
for a reason unrelated to the source.

*(**U2 correction, and it is surplus rationale again — the F1 class.** v4 added that the set "spans
all exports and **both streams** in a single pass." The single-pass fact is true, but **the
cross-stream part cannot affect N6 survivors**: keys are namespaced — `player_season|{block}|…` at
`:659` versus `medical|{block}|…` at `:664` — so a `medical` row can never collide with a
`player_season` key. **Cross-stream ordering is irrelevant here and the claim is withdrawn.** The
in-domain ordering requirement stands.)*
...
block list · **file → block mapping** · file count · byte count · row count per block · **the
raw-header multiset schema identity AND its duplicate/slug-collision validation result** (§4.5) ·
**raw SHA-256 per file** · **semantic digest per block** · explicit schema/coverage result evaluated
**BEFORE** any content comparison.

### 4.5 Schema identity, interval precedence, and 36-block aggregation (R3 + T2)

**⚠ U1 REPAIR — v4's definition was itself defective, and this is a repair of a repair.** v4 said
*"the sorted SET of `_slug`-normalized column names."* **Two independent failures, both demonstrated
rather than argued:**

1. **A set cannot detect a duplicate** — duplicated raw headers collapse to one member, so
   `duplicated ⇒ incomparable` was unenforceable by the very identity meant to enforce it.
2. **`_slug` collides, so renames hide.** `_slug` is
   `re.sub(r"[^0-9a-zA-Z]+", "_", value).strip("_").lower()`, so every non-alphanumeric run maps to
   `_`. **Measured:** `'target-share'`, `'target share'`, `'Target Share'` and `'target_share'` **all
   → `'target_share'`**. A provider renaming a header by separator or case alone would be **invisible**
   under a slug-based identity.

**Schema identity is therefore the ORDER-INSENSITIVE EXACT RAW-HEADER MULTISET** — the sorted tuple
of **raw, un-slugged** column names per block, preserving repeats — **plus two explicit validations
recorded with the result:**

| Validation | On failure |
| :-- | :-- |
| **raw duplicate headers** present | **`incomparable`** |
| **two distinct raw headers slug-colliding** | **`incomparable`** — the production store cannot represent both, so the observation is not trustworthy |

*(Slug-space remains what production stores; **it is not sound as the comparison identity**. Both the
raw multiset identity and the validation result are recorded per §4.4.)*

*(This must also be defined here rather than inherited: `check_player_season_schema` signs schemas
with `sha256("|".join(e.columns))` at `:311`, which is **order-SENSITIVE** and would classify a pure
...
| column **added / removed / renamed / duplicated / slug-colliding** | **`incomparable`** |
...
1. **`incomparable`** — any endpoint invalid, **detectably** incomplete, or unavailable; **or** any
   mismatch in report/filter identity, **raw-header multiset schema identity** (or a failed
   duplicate/slug-collision validation), or the §4.3 provenance triple.
...
· a changed raw-header multiset schema identity, a raw duplicate header, or a slug collision.
**These ⇒ `incomparable` via rule 1.**
