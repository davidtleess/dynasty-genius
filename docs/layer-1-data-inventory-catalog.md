...
| **Final automation classes — B1–B24** | Provisional classes in refresh plan §3; cadence research independently CLEAR at its pin | No canonical stream row carries a FINAL class | refresh plan §1 (seven classes) · cadence artifact + disposition v2 | Claude authors · Gemini facts · Codex reviews | **Every B1–B24 row carries exactly one of the seven classes, recorded as reviewed planning judgment — not as a measured fact** *(M4)* | class ≠ enablement | **OPEN** |
...
| Canonical row/cell | Current measured state | Exact unresolved question / defect | Evidence path or probe | Lane | Binary pass condition | Authority dependency | Status |
| :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| **Cadence fields — ENUMERATED over B1–B24** *(M1)* | Source-publish cadences pinned to primary sources, independently CLEAR; **not yet written onto the canonical rows** | Each of **B1–B24** needs five fields: source-publish cadence · job cadence · freshness expectation · dependency edge · proposed automation class | cadence artifact (CLEAR at pin) + disposition v2 · Gemini job/marker facts · §4.1 | Claude · Gemini facts · Codex | **Every B1–B24 row carries all five fields, each independently verified or explicitly `N/A`/`not scheduled` WITH EVIDENCE. `UNVERIFIED` leaves the row OPEN** *(M4)*. **R3 held: job ≠ freshness ≠ stream cadence** | pinning ≠ scheduling | **OPEN** |
