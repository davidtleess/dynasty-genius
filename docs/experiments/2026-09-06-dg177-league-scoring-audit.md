...
`tests/contract/test_league_scoring_audit.py` (66 tests, every rule RED first). Weekly nflverse totals are the authority
...
**Producer run.** `runs/20260906T215247Z/dg177_league_scoring_audit/` (launch git `634ddf11`, tracked tree clean,
...
`league.json` `d66be804…`. Outputs and their sha256 are in `manifest.json`. Three earlier runs are preserved beside
their notes: `214340Z` FAILED ACCEPTANCE (float sleeper ids resolved nobody; the tool refused, as designed),
`214632Z` and `214808Z` SUPERSEDED (pre-review guards).
...
field-goal muff 2025_10_NO_CAR/2504); 0 missing player ids; 0 events not joinable to a weekly row. Twelve weekly
...
530 rows kept with every original column and exception: 22 from 2025 (18 REG, 4 POST), 508 historical. Nonzero under
the league's individual keys: 36 — the six original nonzero exceptions (2001×3, 2003, 2005 +6.0, 2012 +3.1; identical
under both formulas) and 30 historical rows whose forced-fumble or recovery counts need a special-teams split that
unidentified rows cannot get (`st_split_unknown`, conservatively counted nonzero). Every 2025 quarantined row is inert
under both formulas. No unknown component values.
