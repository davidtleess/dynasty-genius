...
Fixture generator: tests/fixture_generators/build_pfr_fixture.py
...
def fixture_manifest() -> dict[str, Any]:
    if not FIXTURE_MANIFEST.exists():
        pytest.fail(
            ...
            "tests/fixture_generators/build_pfr_fixture.py"
        )
...
def test_the_pinned_shape_still_matches_the_committed_fixture(fixture_payload) -> None:
    ...
    for name in STREAM_NAMES:
        assert set(fixture_payload[name][0]) == set(EXPECTED_COLUMNS[name]), (
            ...
            "regenerate with tests/fixture_generators/build_pfr_fixture.py"
        )
...
def _seed_existing_streams(identity, db_path: Path, raw_root: Path):
    ...
    `tests/fixture_generators/build_pfr_fixture.py`. Injuries is captured in a second run because its rows are
